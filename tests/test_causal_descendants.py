"""Tests for the read-only causal descendant chain endpoint.

The causal endpoint is::

    GET /v1/causal/descendants?replicaId=R&operationId=O&after=N&limit=N

It locates one first-accepted operation by identity (both identities ride
the query string, like the comparison routes) and reports its complete
strict causal descendant chain: the accepted records committed after the
source in the shared log whose clocks strictly dominate the source
operation's clock. Each descendant is classified as ``direct`` (not
dominated by any other strict descendant) or ``transitive``, always
against the complete descendant set. The response carries the source
operation in the archive shape, one page of descendants in global commit
order, the resume cursor, and the more flag as compact canonical JSON
...
class CausalDescendantsStoreTests(unittest.TestCase):
    ...
    def test_strict_descendants_are_reported_in_commit_order(self) -> None:
        ...
        self.assertEqual(
            [
                (d["replicaId"], d["operation"]["operationId"])
    ...
    def test_records_committed_before_the_source_are_excluded(self) -> None:
        store = StateStore()
        # Committed first with a larger clock: it dominates the later
        # source's clock, but only records committed after the source count
        # as descendants.
        ...
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(payload["descendants"], [])
        # The converse: o1's clock does not dominate o2 either.
        status, payload = store.get_causal_descendants("r2", "o2", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(payload["descendants"], [])
    ...
    def test_equal_clock_is_not_strictly_larger(self) -> None:
        ...
        # Same clock components as o2 (missing r2 counts as 0): the clocks
        # are equal, so the later record is not a strict descendant of o2.
        store.apply_operation("r1", operation("o1", "a", "v", {"r1": 1}))
        status, payload = store.get_causal_descendants("r2", "o2", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(payload["descendants"], [])
    ...
    def test_direct_and_transitive_classification(self) -> None:
        store = StateStore()
        store.apply_operation("r1", operation("o1", "a", "v", {"r1": 1}))
        # Dominates the source but is dominated in turn by o4.
        store.apply_operation("r2", operation("o2", "b", "w", {"r1": 1, "r2": 1}))
        # Concurrent with the source: excluded.
        store.apply_operation("r3", operation("o3", "c", "x", {"r2": 1}))
        store.apply_operation("r1", operation("o4", "d", "y", {"r1": 2, "r2": 1}))
        status, payload = store.get_causal_descendants("r1", "o1", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(
    ...
    def test_concurrent_descendants_are_both_direct(self) -> None:
        store = StateStore()
        store.apply_operation("r1", operation("o1", "a", "v", {"r1": 1}))
        store.apply_operation("r2", operation("o2", "b", "w", {"r1": 1, "r2": 1}))
        ...
        self.assertEqual(
            [
                (d["operation"]["operationId"], d["relation"])
                for d in payload["descendants"]
            ],
            [("o2", "direct"), ("o3", "direct")],
        )

    def test_stale_writes_on_other_keys_participate(self) -> None:
        store = StateStore()
        store.apply_operation("r1", operation("s", "k1", "v", {"r1": 1}))
        # First version on "other".
        store.apply_operation(
            "r9", operation("d", "other", "z", {"r1": 1, "r2": 1, "r9": 1})
        )
        # Stale on "other" (d's clock strictly dominates it) but still a
        # first-accepted record and a strict descendant of the source.
        store.apply_operation("r2", operation("stale", "other", "old", {"r1": 1, "r2": 1}))
        # The stale write added no candidate for its key: "other" stays
        # resolved on d's value.
        status_key, state = store.get_state("other")
        self.assertIs(status_key, HTTPStatus.OK)
        self.assertEqual(state["status"], "resolved")
        self.assertEqual(state["value"], "z")
        status, payload = store.get_causal_descendants("r1", "s", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(
            [
                (d["operation"]["operationId"], d["relation"])
    ...
    def test_accepted_repairs_participate(self) -> None:
        ...
        self.assertEqual(
            ...
            [("fix-1", "transitive"), ("o3", "direct")],
        )
        # The repair's own descendant chain is the later record only.
        status, payload = store.get_causal_descendants("r3", "fix-1", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(
            [
                (d["operation"]["operationId"], d["relation"])
    ...
    def test_replays_and_rejected_requests_never_appear(self) -> None:
        ...
        store.apply_operation("r2", operation("o2", "b", "other", {"r1": 1, "r2": 1}))
        status, payload = store.get_causal_descendants("r1", "o1", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(
            payload["descendants"],
    ...
    def test_paging_boundaries(self) -> None:
        store = StateStore()
        store.apply_operation("r0", operation("s", "k", "v", {"r0": 1}))
        for index in range(3):
            store.apply_operation(
                "r1",
                operation(f"o{index}", f"k{index}", "x", {"r0": 1, "r1": index + 1}),
            )
        status, page1 = store.get_causal_descendants("r0", "s", 0, 2)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(len(page1["descendants"]), 2)
        self.assertEqual(page1["cursor"], 2)
        self.assertIs(page1["more"], True)
        status, page2 = store.get_causal_descendants("r0", "s", page1["cursor"], 2)
        self.assertIs(status, HTTPStatus.OK)
        ...
        status, tail = store.get_causal_descendants("r0", "s", 3, 100)
        self.assertIs(status, HTTPStatus.OK)
        ...
        _, whole = store.get_causal_descendants("r0", "s", 0, 100)
        self.assertEqual(
            page1["descendants"] + page2["descendants"], whole["descendants"]
        )

    def test_relation_is_classified_from_the_full_set_not_the_page(self) -> None:
        # A transitive middle record keeps its relation on every page.
        store = StateStore()
        store.apply_operation("r1", operation("s", "k", "v", {"r1": 1}))
        store.apply_operation("r2", operation("o1", "a", "x", {"r1": 1, "r2": 1}))
        store.apply_operation("r2", operation("o2", "b", "y", {"r1": 1, "r2": 2}))
        store.apply_operation("r2", operation("o3", "c", "z", {"r1": 1, "r2": 3}))
        _, whole = store.get_causal_descendants("r1", "s", 0, 100)
        self.assertEqual(
            [d["relation"] for d in whole["descendants"]],
            ["transitive", "transitive", "direct"],
        )
        paged: dict[str, str] = {}
        for after in range(3):
            _, page = store.get_causal_descendants("r1", "s", after, 1)
            (entry,) = page["descendants"]
            paged[entry["operation"]["operationId"]] = entry["relation"]
        self.assertEqual(
            paged,
            {
                d["operation"]["operationId"]: d["relation"]
                for d in whole["descendants"]
            },
        )
    ...
    def test_query_is_read_only(self) -> None:
        ...
        before_audit = store.get_key_audit_digest("b")
        before_state = store.get_state("b")
        before_sync = store.get_sync_operations(0, 100)
        before_archive = store.get_operation("r1", "o1")
        before_ancestors = store.get_causal_ancestors("r2", "o2", 0, 100)
        ...
        self.assertEqual(store.get_key_audit_digest("b"), before_audit)
        self.assertEqual(store.get_state("b"), before_state)
        self.assertEqual(store.get_sync_operations(0, 100), before_sync)
        self.assertEqual(store.get_operation("r1", "o1"), before_archive)
        self.assertEqual(
            store.get_causal_ancestors("r2", "o2", 0, 100), before_ancestors
        )
    ...
    def test_data_file_restart_preserves_descendants(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                recovered.get_causal_descendants("r1", "o1", 0, 100), expected
            )
            self.assertEqual(
...
class HttpCausalDescendantsTests(unittest.TestCase):
    ...
    def test_round_trip(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                descendant(
                    "r2", "o2", "size", "large", {"r1": 1, "r2": 1}, "direct"
                )
            ],
        )
    ...
    def test_non_ascii_strings_are_written_literally(self) -> None:
        self.post_operation("r1", operation("o1", "clé", "bleu", {"r1": 1}))
    ...
    def test_identity_values_are_percent_decoded(self) -> None:
        ...
        status, payload, _, _ = self.get_descendants(
            "?replicaId=r%2F1&operationId=o%2F1"
        )
        self.assertEqual(status, 200)
        self.assertEqual(
            payload["operation"],
    ...
    def test_paging_over_http(self) -> None:
        self.post_operation("r0", operation("s", "k", "v", {"r0": 1}))
        for index in range(3):
            self.post_operation(
                "r1",
                operation(f"o{index}", f"k{index}", "x", {"r0": 1, "r1": index + 1}),
            )
        status, page1, _, _ = self.get_descendants(
            "?replicaId=r0&operationId=s&after=0&limit=2"
        )
        self.assertEqual(status, 200)
        self.assertEqual(len(page1["descendants"]), 2)
        self.assertEqual(page1["cursor"], 2)
        self.assertIs(page1["more"], True)
        status, page2, _, _ = self.get_descendants(
            "?replicaId=r0&operationId=s&after=2&limit=2"
        )
        self.assertEqual(status, 200)
        self.assertEqual(len(page2["descendants"]), 1)
        self.assertEqual(page2["cursor"], 3)
        self.assertIs(page2["more"], False)
        status, tail, _, _ = self.get_descendants(
            "?replicaId=r0&operationId=s&after=3"
        )
        self.assertEqual(status, 200)
        self.assertEqual(tail["descendants"], [])
        self.assertIs(tail["more"], False)

    def test_missing_identities_are_400(self) -> None:
        self.post_operation("r1", operation("o1", "k", "v", {"r1": 1}))
        for query in (
            "",  # no query at all
            "?",
    ...
    def test_bad_query_parameters_are_400(self) -> None:
        ...
        for query in (
            f"?{base}&x=1",
            f"?{base}&replicaId=r2",
            f"?{base}&operationId=o2",
            f"?{base}&after=1&after=2",
            f"?{base}&after=",
            f"?{base}&after",
            f"?{base}&after=-1",
            f"?{base}&after=1.0",
            f"?{base}&after=%201",  # whitespace
            f"?{base}&after=%EF%BC%91",  # non-ASCII digit
            f"?{base}&after=2",  # past the descendant count of 1
            f"?{base}&limit=0",
            f"?{base}&limit=101",
            f"?{base}&limit=",
            f"?{base}&limit=-1",
    ...
    def test_missing_and_extra_segments_are_404(self) -> None:
        ...
        base = "replicaId=r1&operationId=o1"
        for path in (
            f"/v1/causal/descendants/extra?{base}",
            "/v1/causal/descendants/",
            "/v1/causal/descendants/?x=1",
            "/v1/causal",
            "/v2/causal/descendants",
            f"/v2/causal/descendants?{base}",
        ):
    ...
    def test_route_shape_error_beats_query_error(self) -> None:
        self.post_operation("r1", operation("o1", "k", "v", {"r1": 1}))
        for path in (
            "/v1/causal/descendants/extra?replicaId=r1&operationId=o1",
            "/v1/causal/descendants/?x=1",
    ...
    def test_reserved_route_name_does_not_become_a_replica_id(self) -> None:
        # With no query the descendants route is a known shape and answers
        # 400 (missing identities); it must never fall through to the
        # single-chain lookup of replica "descendants".
        status, payload, _, _ = self.get_descendants()
    ...
    def test_rejected_query_reads_and_changes_nothing(self) -> None:
        ...
        before_archive, _, _, _ = self.request("GET", "/v1/replicas/r1/operations/o1")
        before_chain, _, _, _ = self.get_descendants("?replicaId=r1&operationId=o1")
        self.get_descendants("?replicaId=r1&operationId=o1&x=1")
        self.get_descendants("?replicaId=r1&operationId=o1&after=9")
        ...
        after_archive, _, _, _ = self.request("GET", "/v1/replicas/r1/operations/o1")
        after_chain, _, _, _ = self.get_descendants("?replicaId=r1&operationId=o1")
        self.assertEqual(before_metrics, after_metrics)
        self.assertEqual(before_digest, after_digest)
        self.assertEqual(before_archive, after_archive)
        self.assertEqual(before_chain, after_chain)
    ...
    def test_post_to_descendants_route_is_404(self) -> None:
        ...
        status, payload, _, _ = self.request(
            "POST", "/v1/causal/descendants?replicaId=r1&operationId=o1", {}
        )
...
class HttpCausalDescendantsAuthTests(unittest.TestCase):
    ...
    def test_descendants_requires_bearer_token(self) -> None:
        ...
        self.assertEqual(status, 201)
        path = "/v1/causal/descendants?replicaId=r1&operationId=o1"
        for auth in (None, "Bearer nope", "bearer sekret", "Bearer  sekret"):
            status, payload, _ = self.request("GET", path, auth=auth)
            self.assertEqual(status, 401, auth)
            self.assertEqual(payload, {"error": "unauthorized"}, auth)
        status, payload, raw = self.request("GET", path, auth="Bearer sekret")
        self.assertEqual(status, 200)
        self.assertEqual(payload["descendants"], [])
        # A malformed/missing Authorization header still produces the
        # canonical newline body.
        status, _, raw = self.request("GET", path, auth=None)
        self.assertEqual(status, 401)
        self.assertEqual(raw, b'{"error":"unauthorized"}')

    def test_duplicate_authorization_header_is_unauthorized(self) -> None:
        conn = http.client.HTTPConnection("127.0.0.1", self.port, timeout=5)
        conn.putrequest("GET", "/v1/causal/descendants?replicaId=r1&operationId=o1")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            second = serve_once(
                [
                    ...
                    (
                        "GET",
                        "/v1/causal/descendants?replicaId=r2&operationId=absent",
                        None,
                    ),
                    (
                        "GET",
                        "/v1/causal/descendants?replicaId=r1&operationId=o1&x=1",
                        None,
                    ),
                ]
            )
