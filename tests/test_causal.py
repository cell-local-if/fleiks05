"""Tests for the read-only per-operation causal ancestor endpoint.
...
causal predecessors: the accepted records committed before it in the shared
log whose clocks are strictly smaller than the source operation's clock.
Each predecessor is classified as ``direct`` (not dominated by any other
strict predecessor) or ``transitive``. The response carries the source
operation in the archive shape, one page of ancestors in global commit
...
class CausalAncestorsStoreTests(unittest.TestCase):
    """Store-level semantics of the causal-ancestor snapshot."""
    ...
    def test_strict_predecessors_are_reported_in_commit_order(self) -> None:
        ...
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(
            [(a["replicaId"], a["operation"]["operationId"]) for a in payload["ancestors"]],
            [("r1", "o1"), ("r2", "o2")],
        )
    ...
    def test_records_committed_after_the_source_are_excluded(self) -> None:
        ...
        # Committed after the source and strictly smaller: still excluded,
        # because only records before the source commit count.
        store.apply_operation("r2", operation("o2", "a", "x", {"r1": 1, "r2": 1}))
        status, payload = store.get_causal_ancestors("r1", "o1", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(payload["ancestors"], [])

    def test_equal_clock_is_not_strictly_smaller(self) -> None:
        store = StateStore()
        store.apply_operation("r1", operation("o1", "a", "v", {"r1": 1}))
        # Same clock components as o1 (missing r2 counts as 0): the clocks
        # are equal, so o1 is not a strict predecessor of o2.
    ...
    def test_missing_components_count_as_zero(self) -> None:
        store = StateStore()
        store.apply_operation("r1", operation("o1", "a", "v", {"r1": 1}))
        # {"r1": 1} is strictly smaller than {"r1": 1, "r2": 1}.
        store.apply_operation("r2", operation("o2", "b", "w", {"r1": 1, "r2": 1}))
        status, payload = store.get_causal_ancestors("r2", "o2", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(
    ...
    def test_direct_and_transitive_classification(self) -> None:
        store = StateStore()
        store.apply_operation("r1", operation("o1", "a", "v", {"r1": 1}))
        # Dominates o1 and o3: the only direct predecessor of the source.
        store.apply_operation("r2", operation("o2", "b", "w", {"r1": 1, "r2": 1}))
        ...
        self.assertEqual(
            ...
            [
                ...
                ancestor("r3", "o3", "c", "x", {"r2": 1}, "transitive"),
            ],
        )
    ...
    def test_stale_writes_participate(self) -> None:
        ...
        # adds no candidate. It is still a strict predecessor of a later
        # operation — and a transitive one, since o1 dominates it.
        store.apply_operation("r2", operation("o2", "k", "w", {"r1": 1, "r2": 1}))
        store.apply_operation("r3", operation("o3", "a", "x", {"r1": 2, "r2": 1, "r3": 1}))
        status, payload = store.get_causal_ancestors("r3", "o3", 0, 100)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(
            [(a["operation"]["operationId"], a["relation"]) for a in payload["ancestors"]],
            [("o1", "direct"), ("o2", "transitive")],
        )
    ...
    def test_accepted_repairs_participate(self) -> None:
        ...
        self.assertEqual(
            [(a["operation"]["operationId"], a["relation"]) for a in payload["ancestors"]],
            [("o1", "transitive"), ("o2", "transitive"), ("fix-1", "direct")],
        )
    ...
    def test_paging_boundaries(self) -> None:
        ...
        status, page1 = store.get_causal_ancestors("r2", "o9", 0, 2)
        self.assertIs(status, HTTPStatus.OK)
        self.assertEqual(len(page1["ancestors"]), 2)
        self.assertEqual(page1["cursor"], 2)
        self.assertIs(page1["more"], True)
        status, page2 = store.get_causal_ancestors("r2", "o9", page1["cursor"], 2)
        self.assertIs(status, HTTPStatus.OK)
        ...
        status, tail = store.get_causal_ancestors("r2", "o9", 3, 100)
        self.assertIs(status, HTTPStatus.OK)
        ...
        _, whole = store.get_causal_ancestors("r2", "o9", 0, 100)
    ...
    def test_after_past_the_ancestor_count_is_rejected(self) -> None:
        ...
        with self.assertRaises(ValueError):
            store.get_causal_ancestors("r2", "o2", 2, 100)
    ...
    def test_query_is_read_only(self) -> None:
        ...
        store.get_causal_ancestors("r2", "o2", 0, 100)
        store.get_causal_ancestors("absent", "absent", 0, 100)
    ...
    def test_data_file_restart_preserves_ancestors(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.apply_operation("r1", operation("o1", "a", "v", {"r1": 1}))
            store.apply_operation("r2", operation("o2", "b", "w", {"r1": 1, "r2": 1}))
            expected = store.get_causal_ancestors("r2", "o2", 0, 100)
            recovered = StateStore(data_file=data_file)
            self.assertEqual(recovered.get_causal_ancestors("r2", "o2", 0, 100), expected)
            self.assertEqual(
                recovered.get_causal_ancestors("r2", "absent", 0, 100),
                (HTTPStatus.NOT_FOUND, {"error": "not_found"}),
            )


class HttpCausalAncestorsTests(unittest.TestCase):
    ...
    def test_unknown_identity_is_404(self) -> None:
        ...
        for path in ("/v1/causal/r1/absent", "/v1/causal/absent/o1", "/v1/causal/absent/absent"):
    ...
    def test_path_segments_are_percent_decoded(self) -> None:
        ...
        status, payload, _, _ = self.get_causal("r%2F1", "o%2F1")
        self.assertEqual(status, 200)
        self.assertEqual(
            payload["operation"],
    ...
    def test_paging_over_http(self) -> None:
        ...
        status, page1, _, _ = self.get_causal("r2", "o9", "?after=0&limit=2")
        self.assertEqual(status, 200)
        self.assertEqual(len(page1["ancestors"]), 2)
        self.assertEqual(page1["cursor"], 2)
        self.assertIs(page1["more"], True)
        status, page2, _, _ = self.get_causal("r2", "o9", "?after=2&limit=2")
        self.assertEqual(status, 200)
        self.assertEqual(len(page2["ancestors"]), 1)
        self.assertEqual(page2["cursor"], 3)
        self.assertIs(page2["more"], False)
        status, tail, _, _ = self.get_causal("r2", "o9", "?after=3")
        self.assertEqual(status, 200)
        self.assertEqual(tail["ancestors"], [])
        self.assertIs(tail["more"], False)
    ...
    def test_bad_query_parameters_are_400(self) -> None:
        ...
        for query in (
            ...
            "?after=2",  # past the ancestor count of 1
            "?limit=0",
            "?limit=101",
            "?limit=",
            "?limit=-1",
            ...
            status, payload, _, _ = self.get_causal("r2", "o2", query)
    ...
    def test_rejected_query_reads_and_changes_nothing(self) -> None:
        ...
        before_causal, _, _, _ = self.get_causal("r2", "o2")
        self.get_causal("r2", "o2", "?x=1")
        self.get_causal("r2", "o2", "?after=9")
        self.get_causal("absent", "absent", "?x=1")
        ...
        after_causal, _, _, _ = self.get_causal("r2", "o2")
...
class HttpCausalPersistenceTests(unittest.TestCase):
    """The causal report survives a data-file restart unchanged."""
