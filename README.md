...
#### Auditing a peer's confirmation chain

`GET /v1/sync/peers/{peerId}/receipts/audit?after=N&limit=N` gives the sending peer a read-only entry point for auditing the **whole confirmation chain** a peer's receipts form against the shared accepted log. It pages the peer's committed receipts with exactly the same receipt encoding and paging rules as `GET /v1/sync/peers/{peerId}/receipts`, but the integrity verdict and summary always cover the peer's complete receipt history rather than the current page.

- `{peerId}` must be a non-empty percent-decoded path segment, following the same path rules as the receipts route.
- Both `after` and `limit` are required, with the exact same rules as the receipts route: `after` accepts only non-negative ASCII decimal integers (0-based receipt skip cursor), `limit` only ASCII decimal integers from `1` to `100`, repeated or unknown parameters, blank/negative/non-ASCII-decimal values, and an `after` past the peer's current receipt count all return HTTP 400 with `{"error":"invalid_request"}`.
- A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly nine fields: `{"receipts":[...],"nextCursor":N,"hasMore":bool,"algorithm":"sha256","digest":"...","receiptsCount":N,"coverage":{"start":N,"end":N},"conclusion":"ok","anomalies":[...]}`. `receipts`, `nextCursor`, `hasMore`, `algorithm`, `digest`, and `receiptsCount` are computed exactly as on the receipts route — the same receipt items in creation order, the same cursor continuation, and the same whole-history digest, invariant across pages.
- `coverage` is the audited interval of the shared accepted log: `start` is the first receipt's start, derived from its operation count and confirmation cursor (`cursor - len(operations)`), and `end` is the last receipt's confirmation cursor. Every later receipt must begin exactly where the previous one ended, so the segment lengths are strictly continuous from that start; `start == end` for a peer with no receipts (the empty coverage interval).
- `conclusion` is `"ok"` when every receipt begins exactly where the previous one ended — the receipts form one seamless, non-overlapping chain — and every confirmed identity matches the record at the corresponding position of the accepted log; otherwise it is `"broken"`. Empty confirmation segments are legal: a receipt with an empty `operations` list simply starts and ends at the same cursor.
- `anomalies` lists, in audit order (receipt index, then confirmation position), every break found, and is empty when the conclusion is `ok`. Each entry carries the 0-based receipt `index` and falls into one of four disjoint kinds:
  - `{"kind":"gap","index":N,"from":A,"to":B}` — receipt `N` starts at cursor `B`, past the previous receipt's end `A`, so the accepted records in `[A, B)` are never confirmed.
  - `{"kind":"overlap","index":N,"from":B,"to":A}` — receipt `N` starts at cursor `B`, before the previous receipt's end `A`, so `[B, A)` is confirmed twice.
  - `{"kind":"identityMismatch","index":N,"position":P}` — the identity confirmed at absolute log position `P` (including a position outside the accepted log) does not match the accepted record there.
  - `{"kind":"cursorRegression","index":N,"cursor":C}` — receipt `N`'s confirmation cursor `C` is below the previous receipt's cursor (reported once for that receipt instead of a redundant overlap).
- Paging only trims the exported receipts. `coverage`, `conclusion`, `anomalies`, `digest`, and `receiptsCount` are always computed from the peer's complete committed receipt history under one snapshot, so they are identical on every page and on the stable empty tail; an empty receipt set reports a complete, anomaly-free audit of the empty interval with an empty stable page.
- The receipts, the accepted log, the page slice, the coverage, the conclusion, and the anomalies are all computed from one snapshot under the same commit lock used by acknowledgement and checkpoint commits, so the audit always describes a single commit even while receipts are being committed.
- The audit is strictly read-only: it neither advances nor writes the checkpoint, creates no receipt, and changes neither the candidates, the accepted log, transactions, repairs, summaries, metrics counters, nor the data file; it creates no temporary file. A peer that has never registered a checkpoint returns HTTP 404 with `{"error":"not_found"}` without changing any visible state.
- A missing, empty (`/v1/sync/peers//receipts/audit`), multi-segment, or extra (`/v1/sync/peers/{peerId}/receipts/audit/extra`, `/v1/sync/peers/{peerId}/receipts/audit/`, a trailing slash, or a missing segment) path shape returns HTTP 404 with `{"error":"not_found"}`; the route-shape check takes precedence over the query-parameter check and over the unregistered-peer check. Every number in the response is a JSON integer, and strings use the same escaping as the other endpoints. With `--data-file`, the receipts are rebuilt identically during recovery, so the same state yields the same pages, coverage, conclusion, anomalies, and digest before and after a restart. When bearer-token authentication is enabled, the endpoint authenticates like every other non-`/health` route (and `/health` stays anonymous).
