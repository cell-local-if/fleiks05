...
- Scopes authorize HTTP methods: `read` accesses every documented GET and the three read-only POSTs, the batch preview `POST /v1/resolve/auto/plan`, the causal-slice query `POST /v1/states/{key}/causal-at`, the cross-replica comparison `POST /v1/replication/compare`, and the cross-replica synchronization plan `POST /v1/replication/plan`; `write` submits the nine state-changing business POST endpoints; and `admin` covers both classes (it implies read and write) plus the scope-policy reload endpoint and the scope-policy change-audit and audit-verification endpoints. Apart from the read-only plan preview, causal-slice query, cross-replica comparison, and synchronization plan, the state-changing POST endpoints are not reachable with only `read` and the GET endpoints are not reachable with only `write`; neither `read` nor `write` alone reaches the admin-only reload and audit endpoints. `GET /health` stays anonymous in every mode.
...
### Executable replica repair batch (apply)

`POST /v1/replication/apply` executes a repair batch against the local replica: it takes the same cross-replica picture the comparison and the plan consume, plus the local digest the plan was computed against and an ordered action list, and applies the actions in one atomic commit. The request body is a JSON object with exactly four fields:

```json
{"replicaId":"replica-b","snapshot":{"color":[{"clock":{"r2":1},"operationId":"op-2","replicaId":"r2","value":"red"}]},"expectedDigest":"<64 lowercase hex chars>","actions":[{"action":"fetch_remote","key":"color","replicaId":"r2","operationId":"op-2","value":"red","clock":{"r2":1}}]}
```

- `replicaId` and `snapshot` are exactly the comparison/plan request fields: the remote replica's identifier (a non-empty string) and its complete candidate snapshot under the comparison's write constraints. The snapshot is validated and consulted for direction checks — it is never imported wholesale; only the candidates named by `fetch_remote` actions enter the local state.
- `expectedDigest` is the local candidate digest the plan was computed against: exactly 64 lowercase hexadecimal characters, as reported by `GET /v1/replication/snapshot` (`candidateDigest`) and by the comparison's `localDigest` summary field.
- `actions` is an ordered list of 1-100 entries, applied in request order. Every entry carries exactly `action`, `key`, `replicaId`, `operationId`, `value`, and `clock` — the action name plus the candidate identity, value, and clock it acts on — and a `semantic_resolution` entry additionally carries `candidates`, the expected conflicting candidate set (a non-empty list of distinct `{"replicaId","operationId"}` identities, exactly as in a manual resolution). `action` must be `"send_local"`, `"fetch_remote"`, or `"semantic_resolution"` — the plan's action vocabulary; any other direction is rejected. The clock follows the live write constraints (it must contain the entry's own `replicaId`). No two entries may carry the same `(replicaId, operationId)` identity; two entries may share a key (a plan routinely generates several actions for one key).

Malformed JSON, a non-object body, a missing or unknown field, a duplicated field anywhere in the document, a malformed digest, an empty or oversized action list, an illegal action or direction, a structurally illegal entry or clock, a duplicated candidate identity inside a resolution's expected set, or a duplicated action identity all return HTTP 400 with `{"error":"invalid_request"}` and change nothing. The route accepts no query parameters: any parameter returns HTTP 400 with `{"error":"invalid_request"}`, and that check precedes the body check. A missing or extra path segment or a trailing slash (for example `/v1/replication/apply/`) returns HTTP 404 with `{"error":"not_found"}`; the route-shape check takes precedence over the query-parameter and body checks. A `GET` on the path is an unknown route and answers HTTP 404.

The actions are validated **in request order against a staged view** of the store, so the whole batch is fixed before anything is persisted or made visible, and any single failure rejects the entire batch unchanged:

- A `send_local` entry only confirms the local side of the plan: the identity must be a **current** local candidate holding exactly the entry's value and clock, and the remote snapshot must either lack the identity (`missing_remote`) or hold it with the same value under a clock the entry's clock dominates (the plan's local-dominating `clock` direction). An unknown local identity, an overwritten candidate, a remote side that already holds the identity converged or ahead, or a remote value conflict all mean the plan's direction no longer holds: HTTP 409 with `{"error":"apply_conflict"}`. A `send_local` action commits nothing locally — the propagation itself happens between the replicas — so it always counts as replayed.
- A `fetch_remote` entry imports the named remote candidate as one operation with the ordinary write semantics. The remote snapshot must still hold the identity with exactly the entry's value and clock; a missing or changed remote candidate is HTTP 409 with `{"error":"apply_conflict"}`. A locally known identity with identical content is an idempotent replay answered from the committed record.
- A `semantic_resolution` entry commits a manual resolution for the key: the key must currently be in conflict, the entry's `candidates` must name exactly the key's current candidate identities, and the entry clock must dominate every one of those candidates. A missing key, a key whose candidates already agree, or a candidate-set mismatch is HTTP 409 with `{"error":"resolution_conflict"}`; a structurally illegal clock is the `400 invalid_request` above, and a legal clock that does not dominate every expected candidate is also HTTP 400 with `{"error":"invalid_request"}` — a malformed request, not a state conflict. A locally known identity with identical content is an idempotent replay answered from the committed record.
- In every action type, a locally known `(replicaId, operationId)` whose committed content differs from the entry (different key, value, or clock) is HTTP 409 with `{"error":"operation_conflict"}` — the shared identity space of writes, resolutions, transactions, and sync imports.

When at least one action newly commits, the batch then checks `expectedDigest` against the **current committed** candidate snapshot: a mismatch means the plan was computed against a different local state, and the whole batch is rejected with HTTP 409 `{"error":"apply_conflict"}` — nothing changes. A batch in which every action is a replay commits nothing and skips the digest check, so an identical retry of an already-applied batch stays a `200` even though the first apply itself moved the digest.

- At least one action newly accepted: all new operations are persisted in one atomic commit (with `--data-file`, `write temp file → fsync → rename → fsync directory`) **before** the response, exactly like a sync-import batch, and the response is HTTP 201 with `"status":"created"`. The committed operations enter the shared accepted log in request order and flow through sync export, the per-key audit streams and digests, the metrics counters, the verification digest, and the per-operation archive identically to any other accepted operation.
- Every action a replay: HTTP 200 with `"status":"ok"`; nothing is persisted and no temporary file is created.
- A durable-commit failure is HTTP 500 with `{"error":"internal_error"}` and leaves memory, the identity index, and the data file exactly as they were — the batch can be retried.

A success response is a compact UTF-8 JSON object terminated by a single newline, with exactly five fields:

```json
{"accepted":1,"actions":[{"action":"fetch_remote","key":"color","operationId":"op-2","replicaId":"r2","value":"red"}],"replicaId":"replica-b","replayed":0,"status":"created"}
```

- `status`: `"created"` (201) or `"ok"` (200).
- `replicaId`: the requested remote replica id, echoed back.
- `actions`: one result per requested action, in request order, each carrying exactly `action`, `key`, `replicaId`, `operationId`, and the committed `value` (for a replay, the value of the committed record).
- `accepted` and `replayed`: JSON-integer counts of the newly committed actions and the replayed ones.

Every number in the response is a JSON integer; no float, negative zero, or non-finite value can appear. The whole batch runs under the same commit lock used by local writes, sync imports, repairs, and checkpoint commits, so a concurrent reader observes either the old or the new complete state, never half a batch. With `--data-file`, the committed operations recover identically on restart — the replay `200`, the conflict `409`, and the digest guard behave exactly as in a process that never restarted; the apply itself adds no new data-file section. The route shares the common request contract: an illegal `Content-Length` is HTTP 400 `{"error":"invalid_request"}` and a declared length over the body limit is HTTP 413 `{"error":"payload_too_large"}`, both answered **before** reading the body and before authentication; a missing, duplicated, or malformed `Authorization` header or a non-matching bearer token is HTTP 401 `{"error":"unauthorized"}` with the `WWW-Authenticate: Bearer` challenge; and in scope-policy mode an authenticated token lacking the `write` or `admin` scope is HTTP 403 `{"error":"forbidden"}` with no challenge. `/health` stays anonymous.
