...
- On startup the file must parse completely and match the required structure; every stored candidate/operation record must satisfy the same input constraints as live requests, every stored checkpoint must be a non-empty peer id mapped to a non-boolean non-negative integer no greater than the recovered log length, and every stored automatic-resolution binding must carry a non-empty `replicaId`/`operationId`/`key`, a valid clock containing the replica, a supported `policy`, and name a present log operation with matching key and clock. Any corruption, truncation, structural mismatch, unknown top-level key, duplicate/illegal record, or binding/log disagreement makes the service refuse to start — state is never silently dropped or "repaired" by guessing.
...
- Every first-accepted valid operation (the requests that return `201`, including stale writes that add no candidate) is flushed to disk and atomically committed (`write temp file → fsync → rename → fsync directory`) before the response is sent. Identical replays (`200`) append no record; conflicting requests (`409`) change neither memory nor the file. A successful automatic resolution additionally persists its identity binding (key, clock, policy) in the **same** atomic commit as its operation, so the operation and its binding can never diverge on disk after a crash; a failed durable commit rolls both back together. The atomic rename ensures a crash or interrupted write never leaves a partially updated file: the previous or the new complete state survives, never a mix.
...
```json
{"autoResolutions":[{"clock":{"r1":1,"r2":1,"r3":1},"key":"color","operationId":"auto-fix-1","policy":"lowest_identity","replicaId":"r3"}],"checkpoints":{"peer-a":2},"operations":[{"replicaId":"r1","operation":{"clock":{"r1":1},"key":"color","operationId":"op-1","value":"blue"}}],"version":1}
```

The `checkpoints` section is optional and holds sender-side replication cursors (see below); the `autoResolutions` section is optional and holds the automatic-resolution identity bindings described above, one element per successful `POST /v1/states/{key}/resolve/auto`, each carrying `replicaId`, `operationId`, `key`, `clock`, and `policy`. Every binding must name an operation present in the same file's `operations` log with matching key and clock; any mismatch, unknown identity, duplicate, or illegal field makes startup fail rather than silently guessing. A file written before either section existed contains only `version` and `operations`, and recovers with no registered checkpoints and no bindings (an imported automatic resolution replays by operation content instead). `version` stays `1`: the supplemented format is backward compatible, and an old file is upgraded on disk the first time a checkpoint, an automatic resolution, or any other new commit is persisted.
...
- `policy` must be one of the literal strings `"lowest_identity"` or `"highest_identity"`. Any other shape, field, or value is HTTP 400 `{"error":"invalid_request"}`.
- There is no `value` and no `candidates` list: both are determined by the server from the key's current candidates.

Both policies commit only when the key currently holds **different-value** candidates (a key that does not exist or whose candidates all agree is not repaired). The resolution value is then chosen deterministically from the current candidates:

- `lowest_identity`: the value of the candidate with the lexicographically smallest `(replicaId, operationId)`;
- `highest_identity`: the value of the candidate with the lexicographically largest `(replicaId, operationId)`.

Tuple ordering compares `replicaId` first and `operationId` second (so `("r1","zzz")` sorts before `("r2","aaa")`); value ordering never matters. The request clock must dominate **every** current candidate. The resolution is accepted atomically as one operation in the shared commit order, exactly like a manual resolution: the dominated candidates are cleared and the chosen value becomes the only version. Because it is an ordinary accepted operation, it is exported by `GET /v1/sync/operations` (as the chosen `value` together with the request's `replicaId`/`operationId`/`clock` — the sync record carries no `policy`), imported by `POST /v1/sync/operations`, appears in the key's audit stream and both digests, counts in every metrics counter under the initiating replica, is persisted to `--data-file`, and is recovered on restart.

- Success: HTTP 201 with `{"status":"created","key","replicaId","operationId","value","policy"}`, where `value` is the chosen candidate's value and `policy` echoes the requested policy; the field set is identical for both policies.
...
- The identity `(replicaId, operationId)` is bound to **key, clock, and policy**. Replaying the same identity with the same bound key, clock, and policy is HTTP 200 with `{"status":"ok","key","replicaId","operationId","value","policy"}` and appends no log record (the deterministic value is reported back from the committed operation). Any one of key, clock, or policy differing under a known automatic-resolution identity is HTTP 409 `{"error":"operation_conflict"}`, even when both policies would derive the same value; identity replay is answered from the committed operation, so replaying after the key has moved on neither re-resolves nor appends.
- Automatic and manual resolutions share one identity space with ordinary writes. A `(replicaId, operationId)` already used by a write or a manual resolution — including one learned through sync import, whose records carry no policy — follows the shared content rules: same key and clock replay as HTTP 200 (reporting the committed value), anything else is HTTP 409 `{"error":"operation_conflict"}`.
- With `--data-file`, the operation **and** its identity binding (key, clock, policy) are persisted together in one atomic commit (write temp file → fsync → rename → fsync directory) before the 201 response; a durable failure returns HTTP 500 `{"error":"internal_error"}` and rolls back memory, the operation identity, the binding, and the file together, so the request can be retried (a retried identity is not seen as a replay). After a restart the chosen value, the bound policy, replay `200`, and conflict `409` are identical to a process that never restarted. The `autoResolutions` section supplements the version:1 data file (see [Local persistence](#local-persistence-and-recovery-optional)); an older file without it recovers unchanged and is upgraded on disk on the next commit.
