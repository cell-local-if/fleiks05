...
- If `PATH` does not exist, it is created on startup (the parent directory must already exist and pass the startup preflight). The file is created immediately, so an unwritable target fails startup rather than the first write.
- Before recovering an existing file or creating a missing one, startup runs an atomic-commit preflight entirely inside the parent directory. Two uniquely named probe files are created there: a few bytes are written to the source probe, flushed and `fsync`ed, then the source probe is atomically replaced onto the destination probe path (`os.replace`), after which the directory itself is `fsync`ed. Both probe paths — and anything a failed attempt left behind — are removed before startup proceeds. The probe never opens the data file for writing, and never truncates, reorders, or replaces it; after a successful preflight an existing data file's bytes and the memory state recovered from them are unchanged. If any probe step fails, the service exits with code 2 before it starts listening, so the first valid write can never be the first observation that durable commit is unavailable.
- The preflight guarantees only this directory-level create/fsync/rename capability. It does not predict a lock or ACL specific to an existing target file, nor failures that arise after startup (for example the disk becoming unavailable); those still surface at runtime as described below.
