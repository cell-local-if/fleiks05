...
- Scopes authorize HTTP methods: `read` accesses every documented GET and the three read-only POSTs, the batch preview `POST /v1/resolve/auto/plan`, the causal-slice query `POST /v1/states/{key}/causal-at`, the cross-replica comparison `POST /v1/replication/compare`, the cross-replica synchronization plan `POST /v1/replication/plan`, and the multi-replica convergence summary `POST /v1/replication/consensus`; `write` submits the eight state-changing business POST endpoints; and `admin` covers both classes (it implies read and write) plus the scope-policy reload endpoint and the scope-policy change-audit and audit-verification endpoints. Apart from the read-only plan preview, causal-slice query, cross-replica comparison, synchronization plan, and multi-replica convergence summary, the state-changing POST endpoints are not reachable with only `read` and the GET endpoints are not reachable with only `write`; neither `read` nor `write` alone reaches the admin-only reload and audit endpoints. `GET /health` stays anonymous in every mode.
...
### Multi-replica convergence decision summary

`POST /v1/replication/consensus` extends the single-remote comparison to a convergence decision across **multiple** remote replicas at once. The request body is a JSON **array** of between 2 and 100 entries (not an object), kept in request order, each with exactly two fields:

```json
[{"replicaId":"replica-b","snapshot":{"color":[{"clock":{"r1":1},"operationId":"op-1","replicaId":"r1","value":"blue"}]}},{"replicaId":"replica-c","snapshot":{}}]
```

- Every entry carries `replicaId` — the remote replica's identifier, a non-empty string, unique within the array — and `snapshot`, its complete candidate snapshot under **exactly the comparison's constraints** (an object mapping each business key to a non-empty candidate array; each candidate carries exactly `value`, `clock`, `replicaId`, and `operationId` with non-empty strings and a non-empty clock of non-boolean, non-negative JSON integers that contains the candidate's own replica id; an operation identity may appear at most once within one snapshot). Floats (including `1.0` and `-0.0`), the non-finite tokens `NaN`/`Infinity`/`-Infinity`, duplicate fields anywhere in the document, empty or oversized arrays, empty or duplicated replica ids, unknown fields, empty keys or arrays, malformed JSON, and non-array documents all return HTTP 400 `{"error":"invalid_request"}`. The same operation identity may appear in different sources' snapshots — that is exactly what gets aggregated.
- The route accepts no query parameters: any parameter — unknown, repeated, or blank — returns HTTP 400 with `{"error":"invalid_request"}`, and that check precedes the body check. A path missing a segment, carrying an extra or multiple segments, ending in a trailing slash (for example `/v1/replication/consensus/`), or otherwise unknown returns HTTP 404 with `{"error":"not_found"}`; the route-shape decision takes priority over the query-parameter and body checks. A `GET` on the path is an unknown route and answers HTTP 404.

The local current candidates plus every supplied snapshot are aggregated by business key and operation identity `(replicaId, operationId)`; a source that does not hold a key or identity contributes no observation of it. Each observed identity is classified independently:

- **Converged** — every source that holds the identity holds exactly the same content (identical value and clock).
- **Propagatable** (`"propagated"`) — every observation carries the same value with different clocks and exactly one distinct clock **dominates all the other observed clocks** (missing components count as 0). The result names the version to propagate: `"local"` when the local store holds the dominating clock, otherwise the first remote in request order that does; the selected candidate and every distinct dominated clock are retained as `supersededClocks` evidence (a clock shared by several sources is listed once). The same dominating clock held by several sources still identifies exactly one version.
- **Semantic repair** (`"conflict"`) — the observations disagree on value, or their equal-value clocks are concurrent (neither dominates the other; equal clocks with identical content are converged rather than propagated). All conflicting observations and the identity are retained; no value is chosen automatically and no clocks are merged — even when one clock dominates the others, a value disagreement is never auto-propagated.

The remote snapshots are only read — never imported — and the request triggers no repair, transaction, sync, checkpoint, or persistence write. The local candidates are read from one committed snapshot under the same commit lock used by local writes, sync imports, repairs, and checkpoint commits, so the whole summary always describes a single commit: a concurrent commit is observed only as a complete old or new snapshot, never a mix. A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly four fields:

```json
{"keys":[{"identities":[{"decision":{"candidate":{"clock":{"r1":2},"operationId":"op-1","replicaId":"r1","value":"blue"},"source":"local"},"identity":{"operationId":"op-1","replicaId":"r1"},"observations":[{"clock":{"r1":2},"operationId":"op-1","replicaId":"r1","source":"local","value":"blue"},{"clock":{"r1":1},"operationId":"op-1","replicaId":"r1","source":"replica-b","value":"blue"}],"status":"propagated","supersededClocks":[{"r1":1}]}],"key":"color"}],"sources":["local","replica-b","replica-c"],"status":"ok","summary":{"conflicts":0,"converged":0,"sources":3}}
```

- `status`: always `"ok"`.
- `sources`: the source labels in observation order — the literal `"local"` first, then the requested replica ids in request order.
- `keys`: one entry per business key in the union of the local store and every snapshot, sorted lexicographically by key; each entry carries exactly `key` and `identities`. Within a key, identities are sorted by `(replicaId, operationId)`.
- Each identity entry carries `identity` (`{"replicaId","operationId"}`), `status` (`"converged"`, `"propagated"`, or `"conflict"`), and `observations` — one entry per source holding the identity, in source order, each carrying exactly `source`, `value`, `clock`, `replicaId`, and `operationId`. A propagated identity additionally carries `decision` (`{"source","candidate"}`, with the candidate in the usual `{"value","clock","replicaId","operationId"}` shape) and `supersededClocks` (the distinct dominated clocks, sorted by their component values); converged and conflict identities carry neither.
- `summary`: `sources` (the total number of sources — one local plus the number of remotes), `converged` and `conflicts`, JSON integer counts of identities in each class across all keys. Identities present at only one source are converged with themselves.

Every number in the response is a JSON integer (counts and vector-clock ticks); no float, negative zero, or non-finite value can appear. The compact encoding, escaping, and terminator are the same as `GET /v1/replication/snapshot`, and the result is deterministic: the same local state and the same request array produce the identical response.

The query is strictly read-only — it modifies neither memory nor the data file and creates no temporary file. With `--data-file`, the candidate state is rebuilt identically during recovery, so the same local state and the same remote snapshots yield the same summary before and after a restart. The route shares the common request contract: an illegal `Content-Length` is HTTP 400 `{"error":"invalid_request"}` and a declared length over the body limit is HTTP 413 `{"error":"payload_too_large"}`, both answered **before** reading the body and before authentication; a missing, duplicated, or malformed `Authorization` header or a non-matching bearer token is HTTP 401 `{"error":"unauthorized"}` with the `WWW-Authenticate: Bearer` challenge; and in scope-policy mode an authenticated token lacking the `read` or `admin` scope is HTTP 403 `{"error":"forbidden"}` with no challenge (a `read`-only token is sufficient). `/health` stays anonymous.
