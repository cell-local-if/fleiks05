...
### Two-operation causal-difference explanation

`GET /v1/causal/diff?leftReplicaId=R&leftOperationId=O&rightReplicaId=R&rightOperationId=O&after=N&limit=N` reports the minimal locatable explanation of the causal difference between two first-accepted operations. It carries the same four identity query parameters as the comparison endpoint — all required, non-empty, and percent-decoded — and applies the same strict predecessor rules: each side's evidence is the first-accepted records committed before that side's source whose clocks its source clock strictly dominates; the source operation itself never appears, and replays (`200`), rejected requests (`409`/`400`), and uncommitted writes never enter the log. An identity never first-accepted on either side returns HTTP 404 with `{"error":"not_found"}`, and the route-shape check takes precedence over every other check.

A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly nine fields:

```json
{"cursor":2,"difference":{"leftOnly":0,"rightOnly":2,"shared":2},"explanation":[{"from":{"operationId":"o3","replicaId":"r3"},"side":"right","to":{"operationId":"o5","replicaId":"r2"}},{"from":{"operationId":"o4","replicaId":"r1"},"side":"right","to":{"operationId":"o5","replicaId":"r2"}}],"left":{"operation":{"clock":{"r1":2,"r2":1},"key":"d","operationId":"o4","value":"y"},"replicaId":"r1"},"leftOnly":[],"more":false,"right":{"operation":{"clock":{"r1":2,"r2":2,"r3":1},"key":"e","operationId":"o5","value":"z"},"replicaId":"r2"},"rightOnly":[{"operation":{"clock":{"r3":1},"key":"c","operationId":"o3","value":"x"},"relation":"direct","replicaId":"r3"},{"operation":{"clock":{"r1":2,"r2":1},"key":"d","operationId":"o4","value":"y"},"relation":"direct","replicaId":"r1"}],"shared":[{"operation":{"clock":{"r1":1},"key":"a","operationId":"o1","value":"v"},"relation":"transitive","replicaId":"r1"},{"operation":{"clock":{"r1":1,"r2":1},"key":"b","operationId":"o2","value":"w"},"relation":"direct","replicaId":"r2"}]}
```

- `left` and `right`: the two sides' source records in the per-operation archive shape `{"replicaId":R,"operation":{"operationId","key","value","clock"}}`.
- `difference`: `{"shared":S,"leftOnly":L,"rightOnly":R}`, three JSON integer counts over the two sides' complete predecessor identity sets — the same counts `GET /v1/causal/compare` reports, always computed from the full, unpaged sets.
- `shared`, `leftOnly`, and `rightOnly`: the three evidence groups as one merged page. `shared` holds predecessor identities present on both sides, `leftOnly`/`rightOnly` the rest. Within each group the entries follow the shared log's global commit order and keep the comparison predecessor record shape — archive content plus `relation`, classified `"direct"`/`"transitive"` against that group's **complete** record set exactly as the comparison classifies a side, so the label never changes with paging.
- `explanation`: the compressed causal explanation — it only locates, never repairs. For each side it names the exclusive evidence records no other same-side exclusive evidence dominates: the minimal boundary that can no longer be explained away by another one-sided record (the domination test is the same clock comparison behind the `"direct"` relation). Each item has exactly three fields: `from` (the boundary `{"replicaId","operationId"}` identity), `to` (that side's source identity), and `side`, the literal `"left"` or `"right"`. The items from both sides are merged into one global commit order. Shared evidence is never explained — it is identical on both sides.
- `cursor`: the number of merged evidence records skipped after this page — feed it back as the next `after`.
- `more`: whether further merged evidence remains.

Paging walks one stable sequence: the `shared` group first, then `leftOnly`, then `rightOnly`, each in global commit order. `after` (default `0`) is the number of merged records already skipped and `limit` (default `100`, `1`-`100`) bounds the current page across all three groups; each group field in the response carries only that group's slice of the page. An `after` equal to the merged evidence length is a valid empty page, and an `after` past the larger of the groups' combined length returns HTTP 400 with `{"error":"invalid_request"}`. A missing, unknown, or repeated parameter (any of the six names), a blank or missing identity value, a negative or non-ASCII-decimal `after`/`limit`, signs, decimals, whitespace, or a `limit` outside `1-100` likewise return HTTP 400. A missing, empty, or extra path segment (for example `/v1/causal/diff/`, `/v1/causal/diff/extra`, or `/v1/causal`) returns HTTP 404 with `{"error":"not_found"}`; the route-shape check takes precedence over the query-parameter and identity checks.

The `difference` counts and the minimal `explanation` are always computed from the complete predecessor sets, never from the current page, so they do not change with `after`/`limit`. Every number in the response is a JSON integer (counts and the cursor only); strings escape only the quote (`\"`), the backslash (`\\`), and control characters U+0000–U+001F (always as `\u00XX` with lowercase hex) — every other Unicode code point is written literally.

The two source records, both complete predecessor sets, the three groups, the counts, the explanation, and the paging boundaries are computed from one snapshot under the same commit lock used by local writes, sync imports, repairs, and checkpoint commits, so the response always describes a single commit: a read can never observe half an import batch or a partially applied repair. Concurrent commits are observed only as a complete old snapshot or a complete new one. The request is strictly read-only — it modifies neither memory, candidates, logs, checkpoints, audit streams, metrics, nor the data file, and it creates no temporary file. With `--data-file`, the log is rebuilt identically during recovery, so the same recovered state yields the same sources, groups, counts, explanation, and pages before and after a restart. When bearer-token authentication is enabled, an invalid request returns HTTP 401 with `{"error":"unauthorized"}` and the `WWW-Authenticate: Bearer` response header like every other non-`/health` route, and the query stays strictly read-only (and `/health` stays anonymous).
