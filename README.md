...
- Scopes authorize HTTP methods: `read` accesses every documented GET, `write` submits the eight business POST endpoints, and `admin` covers both classes (it implies read and write) plus the scope-policy reload endpoint. The business POST endpoints are not reachable with only `read`, the GET endpoints are not reachable with only `write`, and neither `read` nor `write` alone reaches the admin-only reload endpoint. `GET /health` stays anonymous in every mode.
