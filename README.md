...
- Scopes authorize HTTP methods: `read` accesses every documented GET and the one strictly read-only POST preview `POST /v1/resolve/auto/plan` (it commits nothing), `write` submits the eight business POST endpoints that change state, and `admin` covers both classes (it implies read and write) plus the scope-policy reload endpoint and the scope-policy change-audit and audit-verification endpoints. The state-changing business POST endpoints are not reachable with only `read`, the GET endpoints are not reachable with only `write`, and neither `read` nor `write` alone reaches the admin-only reload and audit endpoints. `GET /health` stays anonymous in every mode.
...
`POST /v1/resolve/auto/plan` lets a caller see, before committing, exactly which value and policy each entry of a batched automatic resolution would select. The request body is the same JSON object and obeys the same constraints as `POST /v1/resolve/auto/batch` — exactly one key `resolutions` holding between 1 and 100 entries in request order, each with exactly `key`, `replicaId`, `operationId`, `clock`, and `policy`; non-empty strings, a structurally valid clock of non-boolean non-negative integer components containing the entry's `replicaId`, `policy` equal to `"lowest_identity"` or `"highest_identity"`, and no two entries naming the same `key` or the same `(replicaId, operationId)` identity. Every numeric value must be an integer: JSON floats (including `1.0`), negative zero (`-0.0`), and `NaN`/`Infinity`/`-Infinity` are all rejected.

The preview is **strictly read-only**. Entries are judged in request order — using the identical staged dry run a real batch commit performs — against one complete committed snapshot, and both a successful plan and a rejected plan change no business state: no candidate, accepted-log record, identity index entry, policy binding, checkpoint, audit record, summary, or data file is written, and no temporary file is created. Repeating a plan therefore returns the same result until a separate commit moves the state.

A successful HTTP 200 response is compact UTF-8 JSON terminated by a single newline, mirroring the batch response's top-level fields with one difference — `status` is always `"planned"` (never `"created"` or `"ok"`):

```json
{"status":"planned","resolutions":[{"key":"color","replicaId":"r3","operationId":"auto-fix-1","value":"blue","policy":"lowest_identity"}],"accepted":1,"replayed":0}
```

- `resolutions` has one result per entry, in request order, each carrying `key`, `replicaId`, `operationId`, the selected string `value`, and `policy`. A new entry's value is the value of the candidate with the smallest or largest `(replicaId, operationId)` under its policy; a same-binding identity reports the originally chosen value from its committed operation.
- `accepted` counts entries that a real batch commit **would newly create** at this snapshot, and `replayed` counts entries whose identity is already committed with the same binding (key, clock, and policy). A plan whose entries are all replays still reports `"status":"planned"` with `accepted` 0 — the preview never reports `"ok"`.
- A known identity with a different binding (a different key, clock, or policy, or an identity committed without a policy binding) returns HTTP 409 with `{"error":"operation_conflict"}`; a same-binding identity reports its original value and counts toward `replayed`.
- An unseen identity for a key that does not exist, whose candidates already agree on one value, whose candidate set has moved, or whose (structurally legal) clock does not dominate every current candidate returns HTTP 409 with `{"error":"resolution_conflict"}`. As with the committing batch, a structurally *illegal* clock is a `400 invalid_request`, distinct from a legal clock that simply fails to dominate the live candidates. Entries are processed in order, so the first failing entry fixes which of the two 409 codes is returned, and the whole plan is rejected either way.
- Malformed JSON, a non-object document, an unknown field at the root or on an entry, an empty or over-100 batch, a duplicate key or identity, an unknown policy, an illegal identity, or an illegal clock (non-object, missing the replica, a negative, boolean, float, `-0.0`, or non-finite component) all return HTTP 400 with `{"error":"invalid_request"}` and no result.
- A missing segment, an extra segment, a trailing slash, or any unknown route returns HTTP 404 with `{"error":"not_found"}`, decided before the query and body checks, so a wrong path shape together with a malformed body or an invalid query is still a 404. The route accepts no query parameters: any parameter — unknown, repeated, blank-named or blank-valued — returns HTTP 400 with `{"error":"invalid_request"}`, and that check precedes the body check even on the correct route.
- The shared Content-Length contract applies unchanged: a missing or malformed declaration returns HTTP 400 `{"error":"invalid_request"}` and a declared length over 1,048,576 bytes returns HTTP 413 `{"error":"payload_too_large"}`, each before authentication and before the body is read.
- Bearer-token authentication is the same contract as every other non-`/health` route: a missing, duplicated, or malformed `Authorization` header or a token mismatch is HTTP 401 with `{"error":"unauthorized"}` and the `WWW-Authenticate: Bearer` challenge, and `GET /health` stays anonymous. Because the endpoint commits nothing, in scope-policy mode it requires the **`read`** scope (or `admin`), exactly like the GET endpoints: a valid token carrying neither returns HTTP 403 with `{"error":"forbidden"}` and **no** challenge, and an unauthenticated or unauthorized request never reads the body. Single-token mode authorizes it with the unrestricted token, as before.

The whole judgement — every entry, the selected values, and the counts — is computed from one snapshot under the same commit lock used by writes, imports, repairs, checkpoints, and the committing batch, so a concurrent commit is observed only as a complete old or complete new snapshot; a result already returned to a caller is not changed by a commit that lands afterwards. With `--data-file`, the preview reads the same recovered state a committing endpoint uses but never writes it, so identical state before and after a restart yields an identical preview (values, policies, counts, and replay decisions included).
