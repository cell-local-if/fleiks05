...
- Scopes authorize HTTP methods: `read` accesses every documented GET and the four read-only POSTs, the batch preview `POST /v1/resolve/auto/plan`, the causal-slice query `POST /v1/states/{key}/causal-at`, the cross-replica comparison `POST /v1/replication/compare`, and the replication sync plan `POST /v1/replication/plan`; `write` submits the eight state-changing business POST endpoints; and `admin` covers both classes (it implies read and write) plus the scope-policy reload endpoint and the scope-policy change-audit and audit-verification endpoints. Apart from the read-only plan preview, causal-slice query, cross-replica comparison, and replication sync plan, the state-changing POST endpoints are not reachable with only `read` and the GET endpoints are not reachable with only `write`; neither `read` nor `write` alone reaches the admin-only reload and audit endpoints. `GET /health` stays anonymous in every mode.
...
- `keys`: one entry per business key in the union of both sides, sorted lexicographically. Each entry's `differences` array covers every candidate identity either side holds for the key, sorted by `(replicaId, operationId)`, and each element keeps both sides' candidate — `local` and `remote`, each `{"value","clock","replicaId","operationId"}` or `null` on the side that lacks the identity — under one `kind` mark:
  ...
  - `"clock"`: both sides hold the identity with the same value but different clocks (one side's clock covers the other's, or the clocks are concurrent). A `"clock"` element also carries `clockDirection`: `"L"` when the local clock dominates the remote one, `"R"` when the remote clock dominates the local one, and `"C"` when the two clocks are concurrent (neither dominates); missing components count as zero, exactly as in the write semantics.
...
### Cross-replica sync planning

`POST /v1/replication/plan` returns a read-only, executable follow-up sync plan between the local current candidates and a remote replica's complete candidate snapshot — the actionable companion to `POST /v1/replication/compare`. The request body is exactly the comparison body's shape: a JSON object with exactly `replicaId` (the remote replica's identifier, a non-empty string) and `snapshot` (the remote's complete candidate state), and every candidate satisfies the same live write constraints — non-empty string `value`, `replicaId`, and `operationId`, and a `clock` whose component values are non-boolean, non-negative JSON integers (floats such as `1.0` and `-0.0` and non-finite values such as `NaN`/`Infinity`/`-Infinity` are rejected) containing the candidate's own replica id, with no operation identity `(replicaId, operationId)` appearing more than once across the whole snapshot.

Malformed JSON, a non-object body, a missing or unknown field, a duplicated field anywhere in the document, an empty key or candidate array, a duplicated candidate identity, or a structurally illegal candidate all return HTTP 400 with `{"error":"invalid_request"}` and change nothing. The route accepts no query parameters: any parameter returns HTTP 400 with `{"error":"invalid_request"}`, and that check precedes the body check. A missing or extra path segment or a trailing slash (for example `/v1/replication/plan/`) returns HTTP 404 with `{"error":"not_found"}`; the route-shape check takes precedence over the query-parameter and body checks.

The remote snapshot is **only planned against** — it is never imported into local state, and the request triggers no repair, transaction, sync, checkpoint, or persistence write. The local candidates are read from one committed snapshot under the same commit lock used by local writes, sync imports, repairs, and checkpoint commits, so the response always describes a single commit even while commits are in flight. A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly four fields:

```json
{"keys":[{"action":"send_local","key":"color","kind":"missing_remote","local":{"clock":{"r1":1},"operationId":"op-1","replicaId":"r1","value":"blue"},"remote":null}],"replicaId":"replica-b","status":"ok","summary":{"actions":1,"identical":false,"localCandidates":1,"localKeys":1,"remoteCandidates":0,"remoteKeys":0}}
```

- `status`: always `"ok"`.
- `replicaId`: the requested remote replica id, echoed back.
- `keys`: one entry per candidate identity that needs an action, ordered by business key lexicographically and then by `(replicaId, operationId)`. Each entry carries the business `key`, the `action` and the `kind` as strings, and both sides' candidate — `local` and `remote`, each `{"value","clock","replicaId","operationId"}` or `null` on the side that lacks the identity. The `kind` marks are the comparison's (`missing_remote`, `missing_local`, `conflict`, `clock`); the `action` is:
  - `"send_local"`: only the local side holds the identity (`missing_remote`), or both hold it with the same value and the local clock dominates (`clock`) — the local version must be sent to the remote.
  - `"fetch_remote"`: only the remote side holds the identity (`missing_local`), or both hold it with the same value and the remote clock dominates (`clock`) — the remote version must be fetched.
  - `"semantic_resolution"`: both sides hold the identity but the values differ (`conflict`), or the values match but the clocks are concurrent (`clock`) — neither version is overwritten; the entry keeps both sides' content for the existing semantic-repair flow.

  Candidates both sides hold with the same value and the same clock generate no action and do not appear in `keys`, so when the two states are identical the plan carries no keys and zero actions.
- `summary`: the convergence totals — `localKeys`/`remoteKeys` and `localCandidates`/`remoteCandidates` count each side's keys and candidate versions; `actions` counts the entries in `keys`; and `identical` reports whether the two candidate states are the same (true precisely when both digests would be equal — in particular when both sides are empty, where `keys` is empty and `actions` is 0).

Every number in the response is a JSON integer and `identical` a JSON boolean; no float, negative zero, or non-finite value can appear. The compact encoding, escaping, and terminator are the same as `POST /v1/replication/compare`.
