...
- Scopes authorize HTTP methods: `read` accesses every documented GET and the three read-only POSTs, the batch preview `POST /v1/resolve/auto/plan`, the causal-slice query `POST /v1/states/{key}/causal-at`, and the cross-replica snapshot comparison `POST /v1/replication/compare`; `write` submits the eight state-changing business POST endpoints; and `admin` covers both classes (it implies read and write) plus the scope-policy reload endpoint and the scope-policy change-audit and audit-verification endpoints. Apart from the read-only plan preview, causal-slice query, and snapshot comparison, the state-changing POST endpoints are not reachable with only `read` and the GET endpoints are not reachable with only `write`; neither `read` nor `write` alone reaches the admin-only reload and audit endpoints. `GET /health` stays anonymous in every mode.
...
### Cross-replica snapshot difference comparison

`POST /v1/replication/compare` is a strictly read-only entry point: a caller submits another replica's complete candidate snapshot and receives the locatable convergence difference against this replica's **current** candidate state. The remote content is never imported into the local store; the comparison triggers no repair, transaction, sync, checkpoint, or audit activity, changes neither memory nor the data file, and creates no temporary file.

The request body is a JSON object with exactly two keys:

```json
{"replicaId":"remote-b","snapshot":{"color":[{"clock":{"r1":1},"operationId":"op-1","replicaId":"r1","value":"blue"}]}}
```

- `replicaId`: a non-empty string identifying the remote replica the snapshot came from. It is echoed back in the report and is not validated against the local replica set.
- `snapshot`: an object mapping non-empty business keys to **non-empty** candidate arrays. Each candidate keeps exactly `value`, `clock`, `replicaId`, and `operationId` and follows the existing write constraints: non-empty strings, and a non-empty clock whose component names are non-empty strings and whose values are non-boolean, non-negative JSON integers, containing the candidate's own `replicaId`. No operation identity `(replicaId, operationId)` may repeat anywhere in the snapshot, including across different keys.
- Every number must be a JSON integer: JSON floats (including `1.0`), negative zero (`-0.0`), and the non-finite tokens `NaN`/`Infinity`/`-Infinity` never enter the comparison or a success response. Malformed JSON, a non-object body, a missing or unknown field, a duplicated JSON field at any level, a structurally illegal clock, an empty key or candidate array, or a repeated identity all return HTTP 400 with `{"error":"invalid_request"}` and change no state.
- The route accepts no query parameters: any parameter — including a repeated name or a blank name/value — returns HTTP 400 with `{"error":"invalid_request"}`, and that check precedes the body check. A missing segment, an extra segment, a trailing slash (for example `/v1/replication/compare/`), or any unknown route returns HTTP 404 with `{"error":"not_found"}`; the route-shape check takes precedence over the query and body checks. A `GET` on the path is an unknown route and answers HTTP 404.

A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly three fields in this order:

```json
{"remoteReplicaId":"remote-b","differences":{"shared":[{"key":"color","identity":{"operationId":"op-1","replicaId":"r1"},"relation":"identical","local":{"clock":{"r1":1},"operationId":"op-1","replicaId":"r1","value":"blue"},"remote":{"clock":{"r1":1},"operationId":"op-1","replicaId":"r1","value":"blue"}}],"localOnly":[{"key":"shape","identity":{"operationId":"op-3","replicaId":"r1"},"relation":"missing_remote","local":{"clock":{"r1":2},"operationId":"op-3","replicaId":"r1","value":"round"},"remote":null}],"remoteOnly":[{"key":"size","identity":{"operationId":"op-9","replicaId":"r9"},"relation":"missing_local","local":null,"remote":{"clock":{"r9":1},"operationId":"op-9","replicaId":"r9","value":"large"}}],"contentConflict":[{"key":"color","identity":{"operationId":"op-2","replicaId":"r2"},"relation":"content_conflict","local":{"clock":{"r2":1},"operationId":"op-2","replicaId":"r2","value":"red"},"remote":{"clock":{"r2":2},"operationId":"op-2","replicaId":"r2","value":"crimson"}}]},"summary":{"localKeys":2,"remoteKeys":2,"localCandidates":3,"remoteCandidates":3,"sameDigest":false,"differences":3}}
```

- `remoteReplicaId`: the request's remote replica id, echoed exactly as sent.
- `differences`: the per-candidate differences grouped by business key into exactly four arrays — `shared`, `localOnly`, `remoteOnly`, and `contentConflict` — each sorted by business key and then by `(replicaId, operationId)` ascending. Every entry carries exactly `key`, `identity` (`{"replicaId","operationId"}`), `relation`, `local`, and `remote`; each present side keeps exactly `value`, `clock`, `replicaId`, and `operationId` (clock components sorted lexicographically), and the absent side is `null`.
  - `shared`: identities present under the same business key on both sides with the same value and clock; `relation` is `"identical"` and both sides are populated.
  - `localOnly`: candidates this replica holds and the remote snapshot does not hold under that key; `relation` is `"missing_remote"` and `remote` is `null`.
  - `remoteOnly`: candidates the remote snapshot holds under a key this replica does not; `relation` is `"missing_local"` and `local` is `null`.
  - `contentConflict`: identities present under the same key on both sides whose content differs. `relation` is `"content_conflict"` when the values differ; when the values agree but the clocks differ it is `"local_clock_covers"` (the local clock dominates the remote clock), `"remote_clock_covers"` (the reverse), or `"clock_divergent"` (neither clock dominates; missing components count as 0).
  Identities match only within the same business key: an identity one side holds under a different key is a missing candidate on each side's own key and is reported on both `localOnly` and `remoteOnly`.
- `summary`: exactly six fields — `localKeys`/`remoteKeys` (key counts on each side), `localCandidates`/`remoteCandidates` (total candidate counts), `sameDigest` (whether the two snapshots produce the same canonical candidate digest as `GET /v1/verification/digest`, computed over the candidate sets alone), and `differences`: the minimal difference count usable to continue synchronization — the number of `localOnly`, `remoteOnly`, and `contentConflict` entries combined (shared candidates excluded).

When both snapshots are empty, the response reports identical digests (`sameDigest` is true), zero counts, and four empty difference groups. When only one side is empty, every key and candidate is reported on that side's `localOnly`/`remoteOnly` group and the other three groups are empty. Every count is a JSON integer (no float, negative zero, or non-finite value can appear), and strings use the same minimal escaping as the other compact endpoints (only the quote, the backslash, and U+0000-U+001F control characters are escaped).

The local snapshot is copied once under the same commit lock used by local writes, sync imports, repairs, and checkpoint commits, so a concurrent commit is observed only as a complete old or new snapshot — the response never mixes half an import batch, a half-applied repair, or a partially recovered state. Repeating the same remote snapshot against unchanged local state yields the identical response. With `--data-file`, the local candidate state is rebuilt identically during recovery, so the same local state and the same remote snapshot produce the same difference report before and after a restart; the query itself writes nothing and creates no temporary file.

The endpoint shares the common request contract: an illegal `Content-Length` is HTTP 400 `{"error":"invalid_request"}` and a declared length over the 1 MiB body limit is HTTP 413 `{"error":"payload_too_large"}`, both answered **before** authentication and before reading the body or any local state; a missing, duplicated, or malformed `Authorization` header or a non-matching bearer token is HTTP 401 with `{"error":"unauthorized"}` and the `WWW-Authenticate: Bearer` challenge; and in scope-policy mode an authenticated token lacking the `read` or `admin` scope is HTTP 403 with `{"error":"forbidden"}` and no challenge. `/health` stays anonymous.
