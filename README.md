...
`GET /v1/causal/compare?leftReplicaId=R&leftOperationId=O&rightReplicaId=R&rightOperationId=O&after=N&limit=N` compares the **strict causal slices** of two first-accepted operations. The four identity parameters are required, are decoded by the standard query rules like every other query value, and must be non-empty; an identity that was never first-accepted returns HTTP 404 with `{"error":"not_found"}`, and the route-shape check takes precedence over every other check.

Each side reuses the single-operation chain of `GET /v1/causal/{replicaId}/{operationId}`: the source record in the archive shape, its strict predecessors in the shared log's global commit order (stale writes and accepted repairs participate like any other committed record), each classified `direct` or `transitive`, and one page of them. `after` (default `0`) is the number of predecessors **both** sides skip together and `limit` (default `100`, `1`-`100`) the shared page size; paging trims only the two predecessor lists — the relation and the difference are always computed over the complete sets.

A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly four fields:

```json
{"difference":{"leftOnly":1,"rightOnly":0,"shared":2},"left":{"ancestors":[{"operation":{"clock":{"r1":1},"key":"a","operationId":"op-1","value":"v"},"relation":"transitive","replicaId":"r1"},{"operation":{"clock":{"r2":1},"key":"b","operationId":"op-2","value":"w"},"relation":"direct","replicaId":"r2"},{"operation":{"clock":{"r1":2},"key":"c","operationId":"op-3","value":"x"},"relation":"direct","replicaId":"r1"}],"cursor":3,"more":false,"operation":{"operation":{"clock":{"r1":2,"r2":1,"r3":1},"key":"d","operationId":"op-4","value":"y"},"replicaId":"r3"}},"relation":"concurrent","right":{"ancestors":[{"operation":{"clock":{"r1":1},"key":"a","operationId":"op-1","value":"v"},"relation":"direct","replicaId":"r1"},{"operation":{"clock":{"r2":1},"key":"b","operationId":"op-2","value":"w"},"relation":"direct","replicaId":"r2"}],"cursor":2,"more":false,"operation":{"operation":{"clock":{"r1":1,"r2":2},"key":"e","operationId":"op-5","value":"z"},"replicaId":"r2"}}}
```

- `left` / `right`: each carries exactly `operation` (the source record in the per-operation archive shape), `ancestors` (one page of that side's strict predecessors), `cursor` (the number of that side's predecessors skipped after this page — feed it back as the next `after`), and `more` (whether further predecessors of that side remain). A side with no predecessors — or an `after` equal to its predecessor count — reports an empty `ancestors` array.
- `relation`: `"left_dominates_right"` when the left source clock dominates the right one, `"right_dominates_left"` for the converse, and `"concurrent"` when neither dominates (equal clocks included).
- `difference`: `{"shared":N,"leftOnly":N,"rightOnly":N}` counting predecessor identities — `(replicaId, operationId)` pairs, deduplicated — over the two **full** predecessor sets: `shared` in both, `leftOnly` only on the left, `rightOnly` only on the right. Every count is a JSON integer.

A missing, repeated, unknown, or empty query parameter, a negative, blank, or non-ASCII-decimal `after`/`limit`, a `limit` outside `1-100`, or an `after` past either side's predecessor count returns HTTP 400 with `{"error":"invalid_request"}` without reading or changing any state. A missing, empty, or extra path segment (for example `/v1/causal/compare/` or `/v1/causal/compare/extra/extra`) returns HTTP 404 with `{"error":"not_found"}`. (A single extra segment after `compare` is the single-operation route for the identity `("compare", <segment>)` and follows that route's rules.)

Every number in the response is a JSON integer; strings escape only the quote (`\"`), the backslash (`\\`), and control characters U+0000–U+001F (always as `\u00XX` with lowercase hex) — every other Unicode code point is written literally.

Both source records, both predecessor lists, the relation, the difference counts, and the paging boundaries are computed from one snapshot under the same commit lock used by local writes, sync imports, repairs, and checkpoint commits, so the response always describes a single commit: a read can never observe half an import batch or a partially applied repair. The request is strictly read-only — it modifies neither memory nor the data file and creates no temporary file. With `--data-file`, the log is rebuilt identically during recovery, so the same recovered state yields the same comparison before and after a restart. When bearer-token authentication is enabled, the endpoint authenticates like every other non-`/health` route.
