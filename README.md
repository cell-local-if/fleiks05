...
`GET /v1/causal/frontier?after=N&limit=N` returns a read-only page of the **causal frontier**: the maximal set of first-accepted operations that no other accepted operation causally dominates. An accepted record stays on the frontier exactly when no other accepted record's clock strictly dominates its own — the same vector-clock comparison as everywhere else (missing components count as 0, and domination already requires the clocks to differ). Two operations whose clocks are equal or mutually non-dominating both stay on the frontier, and the operation kind never influences the judgement: ordinary writes, stale writes whose clock was already dominated, sync-imported records, and accepted conflict repairs (manual or automatic) all participate, while identical replays (`200`), conflicting or malformed requests (`409`/`400`), uncommitted writes, and records whose durable commit failed never enter the log and so never appear.

A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly six fields:

```json
{"operations":[{"operation":{"clock":{"r1":1,"r2":1},"key":"size","operationId":"op-2","value":"large"},"replicaId":"r2"}],"nextCursor":1,"hasMore":false,"algorithm":"sha256","digest":"<64 lowercase hex chars>","operationsCount":1}
```

- `operations`: one page of the frontier in the shared log's global commit order. Each entry preserves the sync-export record shape `{"replicaId":R,"operation":{"operationId","key","value","clock"}}` — identity, key, value, and clock exactly as committed.
- `nextCursor`: the number of frontier records skipped after this page — feed it back as the next `after`.
- `hasMore`: whether further frontier records remain.
- `algorithm` is always `"sha256"`, and `operationsCount` counts the **complete** frontier, never just the page.
- `digest` summarizes the whole frontier, so it is identical on every page. The hash input is a compact UTF-8 JSON array with one element per frontier record in commit order, each serialized with the audit-chain record encoding — the fixed shape `{"replicaId":R,"operation":{"operationId":I,"key":K,"value":V,"clock":C}}` with the clock's component names sorted lexicographically, no whitespace anywhere, and strings escaping only the quote (`\"`), the backslash (`\\`), and control characters U+0000–U+001F (always as `\u00XX` with lowercase hex). An empty frontier hashes the empty array `[]`.

Paging requires both parameters: `after` is the number of frontier records already skipped (a 0-based resume cursor, starting at `0`) and `limit` must be between `1` and `100`; both accept only ASCII decimal integers. A missing or repeated `after`/`limit`, a blank, signed, decimal, whitespace-bearing, or non-ASCII-decimal value, an unknown parameter, or a `limit` outside `1-100` returns HTTP 400 with `{"error":"invalid_request"}` without reading or changing any state. An `after` past the frontier size is HTTP 400; an `after` equal to the frontier size is a valid stable empty page. A missing, empty, or extra path segment (for example `/v1/causal/frontier/`, `/v1/causal/frontier/extra`, or `/v1/causal`) returns HTTP 404 with `{"error":"not_found"}`; the route-shape check takes precedence over the query-parameter check.

Every number in the response is a JSON integer; strings escape only the quote (`\"`), the backslash (`\\`), and control characters U+0000–U+001F (always as `\u00XX` with lowercase hex) — every other Unicode code point is written literally.

The complete frontier, the page slice, the cursor, the remaining flag, the full count, and the digest are computed from one snapshot under the same commit lock used by local writes, sync imports, repairs, and checkpoint commits, so the response always describes a single commit and never observes half an import batch or a partially applied repair. The request is strictly read-only — it modifies neither memory, candidates, logs, checkpoints, receipts, nor the data file, and it creates no temporary file. With `--data-file`, the log is rebuilt identically during recovery, so the same recovered state yields the same frontier, pages, count, and digest before and after a restart. When bearer-token authentication is enabled, the endpoint authenticates like every other non-`/health` route: a missing, duplicated, malformed, or mismatched `Authorization` header is HTTP 401 with the `WWW-Authenticate: Bearer` challenge, in scope-policy mode an authenticated token lacking the `read` or `admin` scope is HTTP 403 without a challenge, and `/health` stays anonymous.
