...
The `checkpoints` section is optional and holds sender-side replication cursors (see below); a file written before checkpoints existed contains only `version` and `operations`, and recovers with no registered checkpoints. The `policies` section is likewise optional and holds the automatic-resolution policy bindings (see below): one `{"replicaId","operationId","policy"}` record per accepted automatic resolution, committed atomically with its operation. The `transactions` section is likewise optional and holds the atomic-transaction bindings (see below): one `{"transactionId","operations"}` record per accepted transaction, committed atomically with its operations. The `acknowledgments` section is likewise optional and holds the consumption-receipt bindings (see below): one `{"peerId","ackId","cursor","operations"}` record per accepted receipt, committed atomically with the checkpoint advance it performs. `version` stays `1`: the supplemented format is backward compatible, and an old file is upgraded on disk the first time a checkpoint (or any other new commit) is persisted.
...
#### Verifiable per-segment consumption receipts

`POST /v1/sync/peers/{peerId}/acknowledge` lets a consuming peer confirm a contiguous segment of the accepted log it picked up past its checkpoint, both advancing that peer's checkpoint and recording a replayable receipt:

```json
POST /v1/sync/peers/peer-a/acknowledge
{"ackId":"ack-1","cursor":2,"operations":[{"replicaId":"r1","operationId":"op-1"},{"replicaId":"r2","operationId":"op-2"}]}
```

- `{peerId}` must be a non-empty percent-decoded path segment, following the same path rules as the checkpoint and pickup routes. The peer must already have a registered checkpoint (`POST /v1/sync/peers/{peerId}/checkpoint`); an unregistered peer returns HTTP 404 with `{"error":"not_found"}`.
- The body must be a JSON object with exactly three keys: `ackId` (a non-empty string naming the receipt), `cursor` (a non-boolean, non-negative integer), and `operations` (a list, in order, of the segment's record identities; the empty list is allowed). Each identity must be an object with exactly `replicaId` and `operationId`, both non-empty strings after decoding, and no identity may repeat within the receipt. At most **100** identities may be acknowledged in one request.
- Starting at the peer's current checkpoint, `operations` must **exactly cover** the contiguous accepted records up to (but not including) `cursor`: the list length must equal `cursor - checkpoint`, and every entry must match the `(replicaId, operationId)` committed at that log position, in global commit order. A missing, extra, reordered, or wrong identity — or a list length that does not match the cursor span — returns HTTP 409 with `{"error":"ack_conflict"}` and changes nothing.
- A `cursor` naming a position past the end of the accepted log can never be exactly covered by accepted records and returns HTTP 409 with `{"error":"ack_conflict"}`. A `cursor` below the peer's current checkpoint returns HTTP 409 with `{"error":"checkpoint_conflict"}`; the stored checkpoint never moves backwards.
- Malformed JSON, a non-object body, an unknown or missing field, a value of the wrong type, an empty `ackId`/`replicaId`/`operationId`, a boolean/string/float/negative `cursor`, a duplicated identity, more than 100 operations, or any query parameter all return HTTP 400 with `{"error":"invalid_request"}` and change no state.
- A missing, empty (`/v1/sync/peers//acknowledge`), or extra (`/v1/sync/peers/{peerId}/acknowledge/extra`, a trailing slash, or a missing segment) path shape returns HTTP 404 with `{"error":"not_found"}`; the route-shape check takes precedence over the query-parameter check, and the empty-peer 404 takes precedence over the query and body checks.
- A valid new receipt returns HTTP 201 with a compact UTF-8 JSON body terminated by a single newline and containing exactly four fields: `{"status":"created","peerId":P,"ackId":A,"cursor":N}`. The peer's checkpoint is advanced to `cursor` in the same commit.
- The same peer posting the same `ackId` with exactly the same `cursor` and `operations` is an idempotent replay: HTTP 200 with `{"status":"ok","peerId":P,"ackId":A,"cursor":N}`; it advances nothing, appends no record, and performs no durable rewrite.
- A known `ackId` for the same peer bound to a different `cursor` or a different `operations` list returns HTTP 409 with `{"error":"operation_conflict"}` and leaves the binding and checkpoint unchanged. Ack ids are independent per peer and distinct from the operation-identity space.

A receipt is not an operation. It changes neither the accepted-operation log nor sync export, the per-key audit, candidate state, or any of the six metrics counters; it is not exported by `GET /v1/sync/operations` and does not appear in audit streams. The segment verification, the checkpoint advance, and the ack-id binding happen **once**, inside the same commit lock used by local writes, import batches, repairs, and checkpoint commits: a concurrent reader observes either the old checkpoint with no receipt or the new checkpoint together with its binding — never half — and a receipt can only name records that are durably accepted.

With `--data-file`, the binding and the checkpoint advance are written to the data file in the same atomic commit protocol (`write temp file → fsync → rename → fsync directory`) before the HTTP 201. A durable failure returns HTTP 500 `{"error":"internal_error"}` and leaves memory, the binding, the checkpoint, and the file exactly as they were, so the request is safely retryable; an exact replay needs no write and still returns 200. A version:1 file written before receipts existed (without the `acknowledgments` section) recovers with no receipts and otherwise unchanged semantics; after recovery the create `201`, replay `200`, binding-conflict `409`, checkpoint-conflict `409`, and log-mismatch `409` decisions are identical to a process that never restarted. The endpoint shares the common request contract: Content-Length is validated before authentication (400/413 first), an invalid or missing bearer token returns HTTP 401 `{"error":"unauthorized"}` without reading the body, and `/health` stays anonymous.
