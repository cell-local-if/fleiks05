...
- Scopes authorize HTTP methods: `read` accesses every documented GET, `write` submits the eight business POST endpoints, and `admin` covers both classes (it implies read and write) plus the scope-policy reload and audit endpoints. The business POST endpoints are not reachable with only `read`, the GET endpoints are not reachable with only `write`, and neither `read` nor `write` alone reaches the admin-only reload and audit endpoints. `GET /health` stays anonymous in every mode.
...
- The replacement is one atomic commit serialized across concurrent reloads: every request observes either the whole old policy or the whole new one, and a request already authenticated continues to execute under the policy revision in force when it authenticated, unaffected by a later reload. Every successful reload also appends its policy-change audit event (see below) in the **same** atomic commit — the new policy and its event become visible together, and with `--data-file` the event is durable before the response is sent. If that durable commit fails, the response is HTTP 500 with `{"error":"internal_error"}` and the **old** policy and the old audit history stay fully in force; the request can be retried. Authentication rejections, permission rejections, and failed reloads (400/401/403/409/500/503) record no event, change no business state, and create no temporary file. A restart still recovers the policy from the configured file (never from the data file); health stays anonymous and every existing write/query/idempotency/persistence behavior is unchanged.

#### Policy-change audit: `GET /v1/admin/scope-policy/audit`

In scope policy mode an administrator can read the committed history of successful hot reloads. The endpoint is published **only** in scope policy mode: single-token mode and anonymous mode answer it with `404 {"error":"not_found"}` (authentication still runs first, so a missing or bad credential is 401 in every mode).

- The request must carry an `admin` token. Authentication failure is HTTP 401 with `{"error":"unauthorized"}` and the `WWW-Authenticate: Bearer` challenge; an authenticated token without the `admin` scope is HTTP 403 with `{"error":"forbidden"}` and no challenge.
- The path must be exactly `/v1/admin/scope-policy/audit` — a missing segment, extra segment, or trailing slash is `404 {"error":"not_found"}`, decided before any query-parameter check.
- The query accepts exactly two parameters, both **required**: `after` (the number of committed events already skipped, a 0-based resume cursor) and `limit` (the page size, 1-100). Each must be a plain ASCII decimal integer: a missing parameter, a negative, blank, signed, or non-ASCII-decimal value, a repeated or unknown parameter, a `limit` outside 1-100, or an `after` past the committed event count is HTTP 400 with `{"error":"invalid_request"}`. An `after` equal to the event count is a valid, stable empty page.
- A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly six fields: `{"events":[...],"nextCursor":N,"hasMore":bool,"algorithm":"sha256","digest":"...","eventsCount":N}`. `events` keeps the reload commit order; each event carries exactly `sequence` (the 1-based position in the history, consecutive from 1), `policyDigest` (the 64-character lowercase hexadecimal SHA-256 of the reloaded policy's **raw UTF-8 bytes**, exactly as the reload reported it), and `tokens` (the reloaded policy's token-entry count). `nextCursor` is the cumulative number of events skipped after this page — feed it back as the next `after`; `hasMore` reports whether further events remain.
- The summary covers the **whole** committed history, never just the page: `eventsCount` counts committed events (not page items), and `digest` is the 64-character lowercase hexadecimal SHA-256 of the canonical digest input — a compact UTF-8 JSON array of all committed events in commit order, each event written with its fields in the fixed order `sequence`, `policyDigest`, `tokens`, numbers as plain JSON integers, strings escaping only the quote, the backslash, and U+0000-U+001F control characters (always as lowercase `\u00xx`, every other code point written literally), and no whitespace anywhere. An empty history hashes the empty array `[]`.
- The page slice, the cursor, the digest, and the count are computed from one snapshot under the same lock that serializes reloads, so a concurrent reload is observed only as a complete old or new history. The query is strictly read-only — it modifies neither the policy, the history, memory, nor the data file, and creates no temporary file. With `--data-file` the events are persisted in the same atomic commit as their reload and recovered on startup (a file written before policy-change auditing existed recovers with an empty history), so the audit is identical before and after a restart.
...
- Neither authentication configuration is ever written to the data file: a `--data-file` restart recovers only operations and the other sections, and the token or scope policy is supplied again (or not) via the command line on each start. The policy-change audit history holds only digests and counts — never tokens or scopes — and old data files stay compatible.
