...
### Per-peer consumption fetch

`GET /v1/sync/peers/{peerId}/operations?after=N&limit=N` returns one page of the accepted operations a sending peer has **not yet consumed**: the shared accepted-operation log from the peer's registered checkpoint onward, in global commit order. The `{peerId}` segment is percent-decoded like every route and must be non-empty.

- The stream contains every first-accepted record past the peer's checkpoint — ordinary writes, stale writes whose clock was already dominated, sync-imported records, and conflict repairs accepted through `POST /v1/states/{key}/resolve` or `POST /v1/states/{key}/resolve/auto`. Identical replays (`200`), conflicting or malformed requests (`409`/`400`), uncommitted requests, and operations whose durable commit failed never enter the log, so they can never appear.
- A peer that has never registered a checkpoint returns HTTP 404 with `{"error":"not_found"}`.
- A successful HTTP 200 response is a compact UTF-8 JSON object terminated by a single newline, with exactly three fields: `{"operations":[...],"nextCursor":N,"hasMore":bool}`. Each operation is `{"replicaId","operation"}` in the same shape as the sync-export record. `nextCursor` is the number of the peer's unconsumed records skipped after this page — feed it back as the next `after` — and `hasMore` reports whether further unconsumed records remain.
- Paging follows the sync-export rules, relative to the checkpoint: `after` is the number of the peer's unconsumed records already skipped (a 0-based resume cursor) and defaults to `0`; `limit` defaults to `100` and must be between `1` and `100`. An `after` equal to the unconsumed record count is a valid empty tail. A missing, repeated, blank, negative, or non-ASCII-decimal `after`/`limit`, a `limit` outside `1-100`, an `after` past the unconsumed record count, or any unknown query parameter returns HTTP 400 with `{"error":"invalid_request"}` without reading or changing any state.
- A missing, empty, or extra path segment (for example `/v1/sync/peers//operations`, `/v1/sync/peers/{peerId}/operations/extra`, or a trailing slash) returns HTTP 404 with `{"error":"not_found"}`; the route-shape check takes precedence over the query-parameter check.
- Every number in the response is a JSON integer; strings escape only the quote (`\"`), the backslash (`\\`), and control characters U+0000–U+001F (always as `\u00XX` with lowercase hex) — every other Unicode code point is written literally.

The checkpoint, the page slice, `nextCursor`, and `hasMore` are read from one snapshot under the same commit lock used by local writes, sync imports, repairs, and checkpoint commits, so the response always describes a single commit: a read can never observe half an import batch or a checkpoint commit halfway through its durable update. The request is strictly read-only — it neither advances nor writes the checkpoint and changes no candidates, audit streams, metrics, summaries, logs, or the data file, and it creates no temporary file. With `--data-file`, the log and the checkpoints are rebuilt identically during recovery (a recovered cursor beyond the log length makes startup fail with exit code 2, exactly like any other rejected data file), so the same state yields the same pages, cursor resume, and error boundaries before and after a restart. When bearer-token authentication is enabled, the endpoint authenticates like every other non-`/health` route (and `/health` stays anonymous).
