...
- Scopes authorize HTTP methods: `read` accesses every documented GET, `write` submits every documented POST, and `admin` covers both classes (it implies read and write). The eight POST endpoints are not reachable with only `read`, and the GET endpoints are not reachable with only `write`. `GET /health` stays anonymous in every mode.
...
#### Hot reload: `POST /v1/admin/scope-policy/reload`

Scope policy mode exposes an atomic runtime reload so an administrator can replace the whole token/scope boundary without a restart. The endpoint is published **only** with `--scope-policy-file`: in anonymous mode and single token mode the path returns `404 not_found` exactly like any unpublished route. It requires a token carrying the `admin` scope, and the request body must be exactly the empty JSON object `{}` — malformed JSON, a non-object body, or any field return `400 invalid_request`.

- A reload rereads **the same file given at startup**, in full; a request can never name another path. The file is only read — it is never written, moved, or replaced, and no temporary file is created. The new content must satisfy the exact startup constraints.
- On success the response is HTTP 200 with exactly `{"status":"reloaded","policyDigest":...,"tokens":N}`: `policyDigest` is the SHA-256 of the file's raw UTF-8 bytes as 64 lowercase hexadecimal characters (byte-for-byte, so surrounding whitespace changes the digest), and `tokens` is the non-negative integer number of token entries. The whole mapping is replaced atomically and concurrent reloads are serialized as complete commits, so every request observes either the old or the new policy — never a partial one. A request that authenticated under the old policy keeps executing with the scopes it authenticated with even if a reload lands while it runs.
- When the file itself is missing, unreadable, not a regular file, or faults on read, the response is HTTP 503 with `{"error":"policy_unavailable"}`. When the file is readable but its content is not a valid policy, the response is HTTP 409 with `{"error":"policy_conflict"}` and the **old permission mapping stays fully in force**. Authentication failures, permission failures, and failed reloads change no business state.
- Error precedence on this route, in order: route shape — missing segments, extra segments, and a trailing slash are `404 not_found` before any query or body check (other HTTP methods are also 404); in scope mode the shared POST Content-Length contract runs before authentication (malformed declaration `400`, over-limit declaration `413`); then authentication (`401`) and the admin scope (`403`), both without reading the body; then query validation — **any** query parameter is `400 invalid_request`, even on the correctly routed path and even when the body is also invalid; and finally body validation (`400`). A restart still recovers the policy from the configured file, and anonymous health plus every existing write, query, idempotency, and persistence behavior is unchanged.
