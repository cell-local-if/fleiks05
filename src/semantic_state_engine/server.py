...
def clock_direction(clock_a: dict[str, int], clock_b: dict[str, int]) -> str:
    """Classify two already-known-different clocks relative to each other.

    Returns ``"L"`` when ``clock_a`` dominates ``clock_b``, ``"R"`` when
    ``clock_b`` dominates ``clock_a``, and ``"C"`` when the clocks are
    concurrent — neither dominates the other. Equal clocks are never
    compared: the callers only ask for a direction when the clocks
    differ. Missing components count as 0, exactly as in
    :func:`clock_dominates`.
    """
    if clock_dominates(clock_a, clock_b):
        return "L"
    if clock_dominates(clock_b, clock_a):
        return "R"
    return "C"
...
def _parse_remote_snapshot(snapshot_raw: Any) -> dict[str, list[dict[str, Any]]]:
    ...
    The snapshot maps each business key to a non-empty array of
    candidates. Each candidate must contain exactly ``value``, ``clock``,
    ``replicaId``, and ``operationId`` and satisfy the live write
    constraints: the value and both identity components are non-empty
    strings, and the clock follows :func:`_validate_clock` (a non-empty
    object whose component values are non-boolean, non-negative JSON
    integers — floats such as ``1.0`` and ``-0.0`` and non-finite values
    are rejected — and which contains the candidate's own replica id). An
    empty key or candidate array, or an operation identity
    ``(replicaId, operationId)`` appearing more than once across the
    whole snapshot raises ValueError. Returns the normalized snapshot;
    it is only validated — never imported into local state.
    """
...
def parse_replication_compare_payload(
    ...
    if not isinstance(replica_id, str) or replica_id == "":
        raise ValueError("replicaId must be a non-empty string")
    snapshot = _parse_remote_snapshot(payload["snapshot"])
    return replica_id, snapshot


REPLICATION_CONSENSUS_MIN_PEERS = 2
REPLICATION_CONSENSUS_MAX_PEERS = 100


def parse_replication_consensus_payload(
    raw: bytes | str | list[Any],
) -> list[tuple[str, dict[str, list[dict[str, Any]]]]]:
    """Parse and validate a multi-replica convergence query body.

    The body must be a JSON array of 2 to 100 entries, each an object
    with exactly ``replicaId`` and ``snapshot``: a non-empty remote
    replica identifier and its complete candidate snapshot. Every
    snapshot follows exactly :func:`_parse_remote_snapshot` — the same
    constraints as the single-remote comparison (each candidate carries
    exactly ``value``, ``clock``, ``replicaId``, and ``operationId``
    under the live write constraints; floats such as ``1.0`` and
    ``-0.0`` and non-finite values are rejected; an operation identity
    may appear at most once within one snapshot). A non-array document,
    an empty or oversized array, an empty or duplicated replica id, a
    missing or unknown field, or a duplicated field anywhere (including
    a duplicated entry key, candidate field, or clock component) raises
    ValueError. Returns the entries in request order as
    ``(replicaId, snapshot)`` pairs. The snapshots are only validated —
    they are never imported into local state.
    """
    if isinstance(raw, (bytes, bytearray)):
        try:
            text = raw.decode("utf-8")
    ...
    if not isinstance(document, list) or not (
        REPLICATION_CONSENSUS_MIN_PEERS
        <= len(document)
        <= REPLICATION_CONSENSUS_MAX_PEERS
    ):
        raise ValueError("body must be an array of 2-100 remote snapshots")
    peers: list[tuple[str, dict[str, list[dict[str, Any]]]]] = []
    seen: set[str] = set()
    for entry in document:
        if not isinstance(entry, dict) or set(entry.keys()) != {"replicaId", "snapshot"}:
            raise ValueError("each entry must have only replicaId and snapshot")
        replica_id = entry["replicaId"]
        if not isinstance(replica_id, str) or replica_id == "":
            raise ValueError("replicaId must be a non-empty string")
        if replica_id in seen:
            raise ValueError(f"duplicate replicaId {replica_id!r}")
        seen.add(replica_id)
        snapshot = _parse_remote_snapshot(entry["snapshot"])
        peers.append((replica_id, snapshot))
    return peers
...
def _parse_expected_candidate_identities(raw: Any) -> list[dict[str, str]]:
    """Validate the expected candidate identity set of a semantic repair.

    The set is a non-empty list of distinct ``{"replicaId", "operationId"}``
    identities — the candidate identities the repair expects its target key
    to currently hold. Raises ValueError on any violation.
    """
...
def parse_replication_apply_payload(
    raw: bytes | str | dict[str, Any],
) -> tuple[str, str, dict[str, list[dict[str, Any]]], list[dict[str, Any]]]:
    """Parse and validate an executable replica-repair batch body.

    The body must be a JSON object with exactly ``replicaId``,
    ``expectedLocalDigest``, ``snapshot``, and ``actions``: the remote
    replica's identifier (a non-empty string), the 64-character lowercase
    hexadecimal digest the caller expects the current committed local
    candidate snapshot to have, the remote's complete candidate snapshot
    (exactly the comparison's constraints, see
    :func:`_parse_remote_snapshot`), and the ordered list of 1-100 repair
    actions. Each action names its direction in ``action`` —
    ``"send_local"``, ``"fetch_remote"``, or ``"semantic_resolution"`` —
    and carries the candidate identity (``replicaId``, ``operationId``),
    ``key``, ``value``, and ``clock`` it acts on; a semantic repair
    additionally carries ``candidates``, the expected current candidate
    identity set of its target key (its ``value`` is the merged value).
    A duplicated field anywhere in the document, an unknown or missing
    field, an unknown direction, a structurally illegal entry, or an
    operation identity ``(replicaId, operationId)`` appearing in more
    than one action raises ValueError. Returns the remote replica id,
    the expected digest, the normalized snapshot, and the normalized
    actions in request order. Nothing is validated against local state
    here — the snapshot is only parsed, never imported.
    """
    if isinstance(raw, (bytes, bytearray)):
        try:
            text = raw.decode("utf-8")
    ...
    if not isinstance(payload, dict) or set(payload.keys()) != {
        ...
        raise ValueError(
            "body must be an object with only replicaId, expectedLocalDigest, "
            "snapshot, actions"
        )
    replica_id = payload["replicaId"]
    if not isinstance(replica_id, str) or replica_id == "":
        raise ValueError("replicaId must be a non-empty string")
    expected_digest = payload["expectedLocalDigest"]
    if not _is_sha256_hex64(expected_digest):
        raise ValueError("expectedLocalDigest must be 64 lowercase hexadecimal characters")
    snapshot = _parse_remote_snapshot(payload["snapshot"])
    actions_raw = payload["actions"]
    if not isinstance(actions_raw, list) or not (
        REPLICATION_APPLY_MIN_ACTIONS <= len(actions_raw) <= REPLICATION_APPLY_MAX_ACTIONS
    ):
    ...
    for entry in actions_raw:
        ...
        if action == "semantic_resolution":
            expected_fields.add("candidates")
        if set(entry.keys()) != expected_fields:
            raise ValueError(
                "each action must have only "
        ...
        if not isinstance(entry_replica, str) or entry_replica == "":
            raise ValueError("replicaId must be a non-empty string")
        operation_id = entry["operationId"]
        if not isinstance(operation_id, str) or operation_id == "":
            raise ValueError("operationId must be a non-empty string")
        ...
        if not isinstance(value, str) or value == "":
            raise ValueError("value must be a non-empty string")
        clock = _validate_clock(entry["clock"], entry_replica)
        identity = (entry_replica, operation_id)
        if identity in identities:
            raise ValueError(f"duplicate identity {identity!r} in batch")
        identities.add(identity)
        ...
        if action == "semantic_resolution":
            normalized["candidates"] = _parse_expected_candidate_identities(
                entry["candidates"]
            )
        actions.append(normalized)
    return replica_id, expected_digest, snapshot, actions
...
CONSENSUS_SOURCE_LOCAL = "local"


def _clock_key(clock: dict[str, int]) -> bytes:
    """A stable comparison key for a vector clock (sorted canonical JSON)."""
    return _ordered_json_bytes(dict(sorted(clock.items())))


def _replication_consensus_report(
    local: dict[str, list[dict[str, Any]]],
    peers: list[tuple[str, dict[str, list[dict[str, Any]]]]],
) -> dict[str, Any]:
    """Build the read-only multi-replica convergence decision summary.

    ``local`` is one committed local candidate snapshot (a copy made by
    the caller under the commit lock) and ``peers`` are the validated
    remote ``(replicaId, snapshot)`` pairs in request order. The local
    store is always observed as the first source, ``"local"``, followed
    by the requested remotes.

    Observations of the same operation identity ``(replicaId,
    operationId)`` are aggregated per business key across every source
    that holds that key:

    - ``"converged"``: every observation carries exactly the same value
      and clock (candidate content identical everywhere).
    - ``"propagated"``: every observation carries the same value with
      different clocks and exactly one distinct clock dominates all the
      other observed clocks (missing components count as 0). The
      decision names the version to propagate — the local version when
      the local source holds the dominating clock, otherwise the first
      remote in request order that does — and keeps every distinct
      dominated clock as ``supersededClocks`` evidence.
    - ``"conflict"``: the observations disagree on value, or their
      equal-value clocks do not dominate one another (concurrent
      clocks). All observations are retained; no value is selected and
      no clocks are merged.

    Keys are sorted lexicographically and identities by
    ``(replicaId, operationId)``; observations within an identity keep
    source order (local first). The summary lists the sources and counts
    the converged and conflict identities. The function is pure: the
    inputs are only read.
    """
    sources: list[str] = [CONSENSUS_SOURCE_LOCAL]
    snapshots: list[tuple[str, dict[str, list[dict[str, Any]]]]] = [
        (CONSENSUS_SOURCE_LOCAL, local)
    ]
    for replica_id, snapshot in peers:
        sources.append(replica_id)
        snapshots.append((replica_id, snapshot))

    all_keys: set[str] = set(local)
    for _, snapshot in peers:
        all_keys.update(snapshot)
    ...
    for key in sorted(all_keys):
        ...
        for replica, operation in sorted(by_identity):
            observations = by_identity[(replica, operation)]
            values = {observation["value"] for observation in observations}
            distinct_clocks: list[dict[str, int]] = []
            for observation in observations:
                if observation["clock"] not in distinct_clocks:
                    distinct_clocks.append(dict(observation["clock"]))
            entry: dict[str, Any] = {
                "identity": {"replicaId": replica, "operationId": operation},
                "status": "",
                "observations": observations,
            }
            if len(values) == 1 and len(distinct_clocks) <= 1:
                # Identical content in every observation: already converged.
                entry["status"] = "converged"
                converged += 1
            elif len(values) == 1:
                # Same value, different clocks: a version can be
                # propagated only when exactly one distinct clock
                # dominates every other observed clock.
                maxima = [
                    clock
                    for clock in distinct_clocks
                    if all(
                        clock == other or clock_dominates(clock, other)
                        for other in distinct_clocks
                    )
                ]
                if len(maxima) == 1:
                    winning_clock = maxima[0]
                    # Observations are in source order, so the first
                    # holder of the winning clock is the local source
                    # when it holds it, otherwise the earliest remote.
                    winner = next(
                        observation
                        for observation in observations
                        if observation["clock"] == winning_clock
                    )
                    superseded = sorted(
                        (
                            dict(clock)
                    ...
                    entry["decision"] = {
                        ...
                        "candidate": {
                            "value": winner["value"],
                            "clock": dict(winner["clock"]),
                            "replicaId": winner["replicaId"],
                            "operationId": winner["operationId"],
                        },
                    }
                    entry["supersededClocks"] = superseded
                else:
                    # Concurrent equal-value clocks: semantic repair.
                    entry["status"] = "conflict"
                    conflicts += 1
            else:
                # Same identity observed with different values: a
                # semantic conflict whatever the clocks say.
                entry["status"] = "conflict"
                conflicts += 1
            identities_out.append(entry)
        groups.append({"key": key, "identities": identities_out})
    return {
        "status": "ok",
        "sources": sources,
        "keys": groups,
...
class StateStore:
    ...
    def compare_replication_snapshot(
        ...
        - ``"clock"``: both sides hold the identity with the same value
          ...
          Such an entry additionally carries ``clockDirection`` —
          ``"L"`` when the local clock dominates the remote clock,
          ``"R"`` when the remote dominates, and ``"C"`` when the two
          clocks are concurrent (missing components count as 0).
        ...
        for key in sorted(set(local) | set(snapshot)):
            ...
            for identity in sorted(set(local_by_id) | set(remote_by_id)):
                ...
                if kind == "clock":
                    entry["clockDirection"] = clock_direction(
    ...
    def plan_replication_sync(
        ...
        """Build a read-only follow-up sync plan against a remote snapshot.
        ...
        never observe half an import batch or a partially applied
        repair — only the old or the new complete state. The remote
        snapshot (already normalized by
        :func:`parse_replication_compare_payload`) is only read: nothing
        is imported, and no repair, transaction, sync, checkpoint, or
        persistence runs. The plan mutates neither memory nor the data
        file and creates no temporary file, so with ``--data-file`` the
        same local state and the same remote snapshot yield the same
        plan before and after a restart.

        The union of business keys is walked in lexicographic order and,
        within each key, the identity union ``(replicaId, operationId)``
        in ascending order. Each identity that cannot already be
        considered converged produces one action:

        - ``"send_local"``: the local side alone holds the identity, or
          the local clock dominates the remote clock.
        - ``"fetch_remote"``: the remote side alone holds the identity,
          or the remote clock dominates the local clock.
        - ``"semantic_resolution"``: both sides hold the identity but
          neither version may be auto-overwritten — either the clocks
          are concurrent (regardless of value) or the values differ; the
          conflicting ``local`` and ``remote`` candidates are both kept
          for the existing semantic-repair flow.

        An identity the two sides hold with the same value and the same
        clock produces no action: when the two candidate states are
        identical the action list is empty and every summary pair is
        equal.
        """
        with self._lock:
            local = {
        ...
        for key in sorted(set(local) | set(snapshot)):
            ...
            for identity in sorted(set(local_by_id) | set(remote_by_id)):
                ...
                kind: str
                if local_candidate is not None and remote_candidate is None:
                    kind = "missing_remote"
                    action = "send_local"
                elif local_candidate is None and remote_candidate is not None:
                    kind = "missing_local"
                    action = "fetch_remote"
                elif local_candidate["value"] == remote_candidate["value"] and (
                    local_candidate["clock"] == remote_candidate["clock"]
                ):
                    # Same value and same clock: already converged, no
                    # action for this identity.
                    continue
                elif local_candidate["value"] != remote_candidate["value"]:
                    # A content conflict is never auto-overwritten, even
                    # when one clock dominates the other; both versions
                    # are retained for semantic resolution.
                    kind = "conflict"
                    action = "semantic_resolution"
                else:
                    # Same value, different clocks: only the causal
            ...
            if entries:
                groups.append({"key": key, "actions": entries})
        local_key_count = len(local)
        remote_key_count = len(snapshot)
        local_candidate_count = sum(len(c) for c in local.values())
        remote_candidate_count = sum(len(c) for c in snapshot.values())
        # identical follows exactly the verification-digest rule the
        # comparison uses: true precisely when the two candidate states
        # are the same (in particular when both are empty).
        local_digest = hashlib.sha256(_verification_digest_input(local)).hexdigest()
        remote_digest = hashlib.sha256(_verification_digest_input(snapshot)).hexdigest()
        return {
            "status": "ok",
            ...
            "keys": groups,
            "summary": {
                "localKeys": local_key_count,
                "remoteKeys": remote_key_count,
    ...
    def replication_consensus(
        self, peers: list[tuple[str, dict[str, list[dict[str, Any]]]]]
    ) -> dict[str, Any]:
        """Summarize convergence across the local store and several remotes.

        The local candidate state is copied under the same commit lock
        used by local writes, sync imports, repairs, and checkpoint
        commits, so the summary always describes a single commit: a
        concurrent commit is observed only as a whole old or whole new
        snapshot, never a mix. The remote snapshots (already normalized
        by :func:`parse_replication_consensus_payload`) are only read:
        nothing is imported, and no repair, transaction, sync,
        checkpoint, or persistence runs. The query mutates neither
        memory nor the data file and creates no temporary file, so with
        ``--data-file`` the same local state and the same remote
        snapshots yield the same summary before and after a restart.

        The decision itself is built by
        :func:`_replication_consensus_report`; see that function for the
        ``converged``/``propagated``/``conflict`` classification, the
        local-first decision tie-break, the superseded-clock evidence,
        and the stable key/identity ordering. The local store is always
        the first source (``"local"``), followed by the requested
        remotes in request order.
        """
        with self._lock:
            local = {
                key: [
        ...
        return _replication_consensus_report(local, peers)
    ...
    def apply_replication_plan(
        ...
        identities, a structurally legal expected digest, and a validated
        remote snapshot. The whole batch runs under the commit lock, so a
        concurrent commit is observed only as a complete old or new
        snapshot, never a mix. First the expected local digest is compared
        with the digest of the current committed candidate state: a
        mismatch rejects the whole batch unchanged with
        ``"apply_conflict"``. The actions are then processed in request
        order against a staged view of the store:

        - a known ``(replicaId, operationId)`` whose committed operation
          carries the same key, value, and clock is a replay and is
          answered from the committed operation without any state check;
        - a known identity with different content is an operation
          conflict;
        - ``send_local``/``fetch_remote`` follow the sync plan's
          directions: an unseen identity makes a send impossible (the
          local side does not hold the candidate) and a fetch requires
          the remote snapshot to still hold the candidate exactly as the
          action claims — a direction that no longer holds or a candidate
          that changed is an ``"apply_conflict"``;
        - ``semantic_resolution`` follows the manual repair semantics: a
          missing target key, a key no longer in value conflict, or an
          expected candidate set that does not match the key's current
          identities is a ``"resolution_conflict"``; a clock that does
          not dominate every expected candidate is a malformed request
          (ValueError), exactly as in :meth:`apply_resolution`.

        A ``send_local`` action changes no local state — the candidate is
        already committed locally — so it is idempotent by construction
        and always counts as replayed. A ``fetch_remote`` action imports
        the remote candidate as one ordinary operation, and a
        ``semantic_resolution`` commits its merged value as one ordinary
        operation, exactly like a sync-import record or a manual repair:
        both flow through the shared accepted log, sync export, the audit
        streams, the metrics, and the data file identically.

        Any failure rejects the whole batch unchanged, however far
        validation got. All new operations are then committed together in
        one atomic commit: the atomic rename is the single durable commit
        point, and memory moves only after it succeeds, so a failed
        durable commit leaves memory, the identity index, and the file
        exactly as before and creates no temporary file.

        Returns ``(status, results, accepted, replayed, error)``: 201
        when at least one action was newly committed or 200 when every
        action was a replay, with ``results`` in request order (each
        carrying ``action``, ``key``, ``replicaId``, ``operationId``, and
        the committed ``value``); or 409 with ``"apply_conflict"`` /
        ``"operation_conflict"`` / ``"resolution_conflict"`` and an
        unchanged store. Raises ValueError when a semantic-repair clock
        does not dominate its expected candidates; raises
        PersistenceError when the durable commit fails.
        """
        with self._lock:
            ...
            for entry in actions:
                ...
                seen = staged_operations.get(identity)
                if seen is not None:
                    # Same replay rule as the write and batch paths: the
                    # identity binding is the committed operation's key,
                    # value, and clock; an identical replay is answered
                    # without any state check, any difference is an
                    # operation conflict.
                    if seen == operation:
                        replayed += 1
                        results.append(
                ...
                if action == "semantic_resolution":
                    ...
                    if current_identities != expected_identities:
                        return HTTPStatus.CONFLICT, [], 0, 0, "resolution_conflict"
                    # A legal clock that fails to dominate the expected
                    # candidates is a malformed request (400), exactly as
                    # in the manual repair flow, not a state conflict.
                    if not all(
                        clock_dominates(operation["clock"], c["clock"])
                        for c in current
                    ):
                        raise ValueError(
                            "clock does not dominate every expected candidate"
                        )
                elif action == "send_local":
                    # An unseen identity is never a current local
                    # candidate, so the planned send direction no longer
                    # holds (the candidate moved or never existed).
                    return HTTPStatus.CONFLICT, [], 0, 0, "apply_conflict"
                else:
                    # fetch_remote: the remote snapshot must still hold
                    # the candidate exactly as the action claims. The
                    # local side cannot hold the identity (it is unseen),
                    # so a match makes the plan's fetch direction hold.
                    remote_candidate = next(
                        (
                            candidate
                            for candidate in snapshot.get(key, [])
                            if (candidate["replicaId"], candidate["operationId"])
                            == identity
                        ),
                        None,
                    )
            ...
            # Commit every new operation together. The atomic rename is
            # the single durable commit point; memory and the identity
            # index move only after it succeeds, so a failed durable
            ...
            if self._data_file is not None:
                ...
                self._accepted.extend(new_records)
            for record_replica, record in new_records:
                self._operations[(record_replica, record["operationId"])] = record
                op_key = record["key"]
                self._candidates[op_key] = self._next_candidates(
                    self._candidates.get(op_key, []), record_replica, record
                )
            return HTTPStatus.CREATED, results, accepted, replayed, None
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_replication_plan_post(self) -> None:
        ...
        # never imported, and no repair, transaction, sync, checkpoint, or
        # persistence runs. The report follows the compact-single-line
        # contract: canonical UTF-8 JSON, one trailing newline, counts
        # only as JSON integers.
        if not parse_metrics_query(urlsplit(self.path).query):
            self._json_canonical_newline(
                HTTPStatus.BAD_REQUEST, {"error": "invalid_request"}
    ...
    def _handle_replication_consensus_post(self) -> None:
        ...
        # fall through to the generic 404 before any of those. The route
        # accepts no query parameters, and that check precedes the body
        # check. The query is strictly read-only: every remote snapshot is
        # validated and aggregated against one committed local snapshot but
        # never imported, and no repair, transaction, sync, checkpoint, or
        # persistence runs. The report follows the compact-single-line
        ...
        if raw is None:
            return
        try:
            peers = parse_replication_consensus_payload(raw)
        except ValueError:
            self._json_canonical_newline(
                HTTPStatus.BAD_REQUEST, {"error": "invalid_request"}
            )
        ...
        self._json_canonical_newline(
            HTTPStatus.OK,
            self._store.replication_consensus(peers),
        )
    ...
    def _handle_replication_apply_post(self) -> None:
        ...
        # fall through to the generic 404 before any of those. The route
        # accepts no query parameters, and that check precedes the body
        # check. The batch executes against one committed snapshot: the
        # expected local digest is verified first, then every action is
        # validated in request order against a staged view, and all new
        # operations commit together atomically. The response follows the
        # compact-single-line contract: canonical UTF-8 JSON, one trailing
        ...
        if raw is None:
            return
        try:
            replica_id, expected_digest, snapshot, actions = (
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if (
            ...
            if is_scope_policy_reload_post:
                ...
                if not self._require_scope(SCOPE_READ):
                    return
            elif is_replication_plan_post:
                # The follow-up sync plan only observes one complete
                ...
                # The multi-replica convergence summary only aggregates
                # the supplied snapshots against one committed local
                # snapshot and never imports them, so it is gated like
                # the comparison and plan: a read or admin scope
                # suffices.
                if not self._require_scope(SCOPE_READ):
                    return
